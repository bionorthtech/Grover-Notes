...
describe('shouldAutoLinkGroverHref', () => {
  it('rejects plain filename-like text', () => {
    expect(shouldAutoLinkGroverHref({ raw: 'AGENTS.md' })).toBe(false)
    expect(shouldAutoLinkGroverHref({ raw: 'docs/README.md' })).toBe(false)
  })

  it('keeps normal url-like values eligible for autolinking', () => {
    expect(shouldAutoLinkGroverHref({ raw: 'https://example.com/docs' })).toBe(true)
    expect(shouldAutoLinkGroverHref({ raw: 'example.com' })).toBe(true)
    expect(shouldAutoLinkGroverHref({ raw: 'example.com/README.md' })).toBe(true)
  })
})
