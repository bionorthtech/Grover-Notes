...
const noteEntry = makeEntry({ path: '/vault/note/welcome-to-grover.md', filename: 'welcome-to-grover.md', title: 'Welcome to Grover', isA: 'Note' })
...
describe('findEntryByTarget', () => {
  ...
  it('matches by relative path (folder/slug)', () => {
    expect(findEntryByTarget(allEntries, 'note/welcome-to-grover')).toBe(noteEntry)
  })

  it('matches by relative path with pipe syntax', () => {
    expect(findEntryByTarget(allEntries, 'note/welcome-to-grover|Welcome!')).toBe(noteEntry)
  })
...
describe('resolveWikilinkColor', () => {
  ...
  it('resolves relative-path wikilink target to correct type color', () => {
    const result = resolveWikilinkColor(allEntries, 'note/welcome-to-grover')
    expect(result.isBroken).toBe(false)
    expect(result.color).toBe('var(--swatch-blue)')
  })
