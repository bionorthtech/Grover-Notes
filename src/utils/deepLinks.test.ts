...
import {
  buildGroverDeepLinkForEntry,
  parseGroverDeepLink,
  relativePathForVaultItem,
  resolveGroverDeepLink,
  vaultDeepLinkSlug,
  type DeepLinkVault,
} from './deepLinks'
...
describe('Grover deep links', () => {
  it('builds readable links with extensions and encoded path segments', () => {
    const result = buildGroverDeepLinkForEntry({
    ...
    expect(result).toEqual({
      ok: true,
      url: 'grover://work-vault/sponsorships/Acme%20call%20%231.md',
    })
  })

  it('round-trips unicode and URL-reserved path characters', () => {
    const url = 'grover://work-vault/books/Caf%C3%A9%20%26%20notes%3F%25.md'
    expect(parseGroverDeepLink({ rawUrl: url })).toEqual({
      ok: true,
  ...
  it('resolves generated collision-safe slugs without opening the wrong vault', () => {
    ...
    expect(resolveGroverDeepLink({ rawUrl: `grover://${slug}/note.md`, vaults: [first, second] })).toEqual({
      ok: true,
  ...
  it('rejects ambiguous handwritten base slugs', () => {
    ...
    expect(resolveGroverDeepLink({ rawUrl: 'grover://work/note.md', vaults: [first, second] })).toEqual({
      ok: false,
  ...
  it('rejects unknown and unavailable vaults', () => {
    expect(resolveGroverDeepLink({ rawUrl: 'grover://missing/note.md', vaults: [workVault] })).toEqual({
      ok: false,
      error: 'unknown_vault',
    })
    expect(resolveGroverDeepLink({
      rawUrl: 'grover://work-vault/note.md',
  ...
  it('rejects path traversal and encoded separators', () => {
    expect(parseGroverDeepLink({ rawUrl: 'grover://work-vault/../secret.md' })).toEqual({
      ok: false,
      error: 'unsafe_path',
    })
    expect(parseGroverDeepLink({ rawUrl: 'grover://work-vault/folder%2Fsecret.md' })).toEqual({
      ok: false,
      error: 'unsafe_path',
    })
  })
  ...
  it('requires target files to stay inside a known vault root', () => {
    ...
    expect(buildGroverDeepLinkForEntry({
