...
function mockAgentResponse(agent: AiAgentId, message: string): string {
  ...
  if (message.indexOf(CONVERSATION_HISTORY_OPEN_MARKER) >= 0) {
    ...
    return `[mock-${agentLabel.toLowerCase()} turns=${turnCount}] You asked: "${lastUserMsg}" — This note is related to [[Build Grover App]] and [[Matteo Cellini]].`
  }
  return `[mock-${agentLabel.toLowerCase()}] You said: "${message}" — This note is related to [[Build Grover App]] and [[Matteo Cellini]].`
}
