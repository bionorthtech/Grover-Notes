...
function expectNoteWindowRoute(parsed: URL): void {
  ...
  expect(parsed.searchParams.get('vault')).toBe('/Users/luca/Grover Vault')
...
describe('openNoteWindow', () => {
  ...
  it('builds a root-app route that preserves the note window params', () => {
    const url = buildNoteWindowUrl('/vault/Folder/My Note.md', '/Users/luca/Grover Vault', 'AI / ML')
    const parsed = new URL(url, 'https://grover.localhost')

    expectNoteWindowRoute(parsed)
  })

  it('resolves the runtime route against the current app origin', () => {
    const url = buildRuntimeNoteWindowUrl('/vault/Folder/My Note.md', '/Users/luca/Grover Vault', 'AI / ML')
  ...
  it('opens a new Tauri window with the encoded note route', async () => {
    ...
    await openNoteInNewWindow('/vault/Folder/My Note.md', '/Users/luca/Grover Vault', 'AI / ML')
    const expectedUrl = `${window.location.origin}/?window=note&path=%2Fvault%2FFolder%2FMy+Note.md&vault=%2FUsers%2Fluca%2FGrover+Vault&title=AI+%2F+ML&windowLabel=note-1776182400000`
    ...
    expect(JSON.parse(localStorage.getItem('grover:note-window:note-1776182400000') ?? '{}')).toEqual({
      notePath: '/vault/Folder/My Note.md',
      vaultPath: '/Users/luca/Grover Vault',
      noteTitle: 'AI / ML',
    })
  })
