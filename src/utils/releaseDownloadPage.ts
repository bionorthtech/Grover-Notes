const RELEASE_HISTORY_URL = 'https://grover.md/releases/'
const DOWNLOAD_FRAME_NAME = 'grover-download-frame'
const WINDOWS_MANAGED_INSTALL_NOTE =
  'Windows installers are Authenticode-signed. Company-managed devices may still require IT to approve the Grover publisher before first install.'
...
const REDIRECT_PAGE_STYLES = `
    ...
    @media (prefers-color-scheme: dark) {
      :root {
        ...
        --download-accent: #5fc98c;
        --download-accent-hover: #84dba8;
...
function buildStableDownloadPageContent(
  ...
  if (Object.keys(downloads).length > 0) {
    return {
      ...
      message: 'Preparing the latest stable Grover download for your platform.',
      shouldRedirect: true,
      title: 'Grover Stable Download',
    }
  }

  return {
    helperText: 'Use the button below to check the latest release history.',
    message: 'No stable Grover downloads are available yet.',
    shouldRedirect: false,
    title: 'Grover Stable Download Unavailable',
  }
}
...
function buildDownloadFrameMarkup(downloads: StableDownloadTargets): string {
  ...
  return `<iframe title="Grover installer download" name="${DOWNLOAD_FRAME_NAME}" sandbox="allow-downloads" hidden></iframe>`
}
    ...
    <script>
      ...
      function downloadMessage(target, requiresMacChoice, requiresWindowsChoice) {
        ...
        if (requiresWindowsChoice) {
          return 'Use the signed Windows installer link below. Company-managed devices may require IT approval of the Grover publisher.';
        }

        return 'Starting the latest stable Grover download for ' + target.label + '.';
      }
