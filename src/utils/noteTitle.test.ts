...
describe('extractH1TitleFromContent', () => {
  ...
  it('preserves plain square brackets in the H1', () => {
    const content = '# [26Q2] Grover MVP'
    expect(extractH1TitleFromContent(content)).toBe('[26Q2] Grover MVP')
  })
...
describe('deriveDisplayTitleState', () => {
  ...
  it('keeps plain square brackets when deriving the display title from H1', () => {
    const content = '# [26Q2] Grover MVP\n\nBody'
    expect(deriveDisplayTitleState({ content, filename: 'grover-mvp.md' })).toEqual({
      title: '[26Q2] Grover MVP',
      hasH1: true,
    })
  })
})
