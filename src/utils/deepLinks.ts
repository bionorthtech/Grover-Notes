...
export const GROVER_DEEP_LINK_SCHEME = 'grover'
...
interface GroverDeepLinkResolveInput extends GroverDeepLinkInput {
  vaults: readonly DeepLinkVault[]
}

interface GroverDeepLinkBuildInput extends VaultPathLookupInput {
...
function rawPathnameForGroverUrl({ rawUrl }: GroverDeepLinkInput): string | null {
  return rawUrl.match(/^grover:\/\/[^/?#]+(\/[^?#]*)/iu)?.[1] ?? null
}
...
function resolveVaultForSlug({ slug, vaults }: VaultSlugLookupInput): ResolvedGroverDeepLink | DeepLinkVault {
...
function resolvedAvailableVault(vault: DeepLinkVault): DeepLinkVault | ResolvedGroverDeepLink {
  return vault.available === false ? { ok: false, error: 'unavailable_vault' } : vault
}

export function parseGroverDeepLink({ rawUrl }: GroverDeepLinkInput): ParsedGroverDeepLink {
  let parsed: URL
  try {
  ...
  if (parsed.protocol !== `${GROVER_DEEP_LINK_SCHEME}:`) return { ok: false, error: 'invalid_scheme' }
  if (!parsed.hostname) return { ok: false, error: 'missing_vault' }

  const rawPathname = rawPathnameForGroverUrl({ rawUrl })
...
export function resolveGroverDeepLink({ rawUrl, vaults }: GroverDeepLinkResolveInput): ResolvedGroverDeepLink {
  const parsed = parseGroverDeepLink({ rawUrl })
...
export function buildGroverDeepLinkForEntry({
  entry,
  vaultPath,
  vaults,
}: GroverDeepLinkBuildInput): BuiltGroverDeepLink {
  ...
  return {
    ok: true,
    url: `${GROVER_DEEP_LINK_SCHEME}://${slug}/${encodeRelativePath({ path: relativePath })}`,
  }
}
