...
describe('evaluateView', () => {
  ...
  it('filters by relationship contains wikilink', () => {
    const entries = [
      makeEntry({ title: 'Match', relationships: { 'Related to': ['[[grover-app|Grover App]]', '[[other]]'] } }),
      makeEntry({ title: 'No match', relationships: { 'Related to': ['[[something]]'] } }),
      makeEntry({ title: 'No rels', relationships: {} }),
    ]
    expectFilterTitles({ field: 'Related to', op: 'contains', value: '[[grover-app]]' }, entries, ['Match'], 'Related')
  })
