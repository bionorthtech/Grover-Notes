...
describe('release workflow macOS artifact names', () => {
  ...
  it('publishes versioned Silicon and Intel artifact names', () => {
    ...
    expect(alphaWorkflow).toContain(
      'Grover_${{ needs.version.outputs.version }}_macOS_Silicon.app.tar.gz',
    )
    expect(alphaWorkflow).toContain(
      'Grover_${{ needs.version.outputs.version }}_macOS_Intel.app.tar.gz',
    )
    ...
    expect(stableWorkflow).toContain(
      'Grover_${{ needs.version.outputs.version }}_macOS_Silicon.dmg',
    )
    expect(stableWorkflow).toContain(
      'Grover_${{ needs.version.outputs.version }}_macOS_Intel.dmg',
    )
  })
  ...
  it('returns stable downloads for each supported desktop platform when present', () => {
    expect(
      extractStableDownloadTargets({
        platforms: {
          'darwin-aarch64': {
            download_url: 'https://example.com/Grover-aarch64.dmg',
          },
          'darwin-x86_64': {
            download_url: 'https://example.com/Grover-x64.dmg',
          },
          'linux-x86_64': {
            download_url: 'https://example.com/Grover.AppImage',
          },
          'windows-x86_64': {
            url: 'https://example.com/Grover-setup.exe',
          },
        },
      }),
    ).toMatchObject({
      'darwin-aarch64': {
        ...
        url: 'https://example.com/Grover-aarch64.dmg',
      },
      'darwin-x86_64': {
        label: 'macOS Intel',
        url: 'https://example.com/Grover-x64.dmg',
      },
      'linux-x86_64': {
        label: 'Linux AppImage',
        url: 'https://example.com/Grover.AppImage',
      },
      'windows-x86_64': {
        label: 'Windows',
        url: 'https://example.com/Grover-setup.exe',
...
describe('buildStableDownloadRedirectPage', () => {
  it('builds a redirect page with platform-specific download links', () => {
    const html = buildStableDownloadRedirectPage({
      'darwin-aarch64': {
        buttonLabel: 'Download Grover for macOS Apple Silicon',
        label: 'macOS Apple Silicon',
        url: 'https://example.com/Grover-aarch64.dmg',
      },
      'darwin-x86_64': {
        buttonLabel: 'Download Grover for Intel Mac',
        label: 'macOS Intel',
        url: 'https://example.com/Grover-x64.dmg',
      },
      'windows-x86_64': {
        buttonLabel: 'Download Grover for Windows',
        label: 'Windows',
        url: 'https://example.com/Grover-setup.exe',
      },
    })

    expect(html).toContain('Grover Stable Download')
    expect(html).toContain('DOWNLOAD_TARGETS')
    expect(html).toContain('Download Grover for Windows')
    expect(html).toContain('Windows installers are Authenticode-signed')
    expect(html).toContain('Download Grover for macOS Apple Silicon')
    expect(html).toContain('Download Grover for Intel Mac')
    ...
    expect(html).toContain('grover-download-frame')
  ...
  it('starts platform downloads without navigating away from the download page', () => {
    ...
    expect(html).toContain('name="grover-download-frame"')
    expect(html).toContain('target="grover-download-frame"')
  ...
  it('builds a fallback page when no stable downloads exist yet', () => {
    ...
    expect(html).toContain('Grover Stable Download Unavailable')
    expect(html).toContain('View release history')
    expect(html).toContain('https://grover.md/releases/')
    expect(html).not.toContain('https://bionorthtech.github.io/grover-notes/')
...
describe('resolveStableDownloadTargets', () => {
  it('falls back to stable release assets when latest.json is incomplete', () => {
    ...
    const releasesPayload = [
      {
        ...
        assets: [
          {
            name: 'Grover_x64.dmg',
            browser_download_url: 'https://example.com/Grover-x64.dmg',
          },
          {
            name: 'Grover-setup.exe',
            browser_download_url: 'https://example.com/Grover-setup.exe',
          },
          {
            name: 'Grover.AppImage',
            browser_download_url: 'https://example.com/Grover.AppImage',
          },
          {
            name: 'Grover.rpm',
            browser_download_url: 'https://example.com/Grover.rpm',
          },
        ],
      },
    ]
    ...
    expect(extractStableDownloadTargetsFromReleases(releasesPayload)).toMatchObject({
      ...
      'linux-x86_64-rpm': {
        label: 'Linux RPM',
        url: 'https://example.com/Grover.rpm',
      },
      'windows-x86_64': {
        url: 'https://example.com/Grover-setup.exe',
      },
  ...
  it('keeps AppImage as the Linux auto-download while exposing RPM manually', () => {
    const html = buildStableDownloadRedirectPage({
      'linux-x86_64': {
        buttonLabel: 'Download Grover AppImage for Linux',
        label: 'Linux AppImage',
        url: 'https://example.com/Grover.AppImage',
      },
      'linux-x86_64-rpm': {
        buttonLabel: 'Download Grover RPM for Linux',
        label: 'Linux RPM',
        url: 'https://example.com/Grover.rpm',
      },
    })
