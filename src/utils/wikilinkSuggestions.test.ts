...
describe('preFilterWikilinks', () => {
  const items: WikilinkBaseItem[] = [
    makeItem('Build Grover App', ['grover-app'], 'Project'),
  ...
  it('returns matches for query of exactly MIN_QUERY_LENGTH', () => {
    ...
    expect(result.some(r => r.title === 'Build Grover App')).toBe(true)
  })
  ...
  it('matches on group', () => {
    ...
    expect(result).toHaveLength(1)
    expect(result[0].title).toBe('Build Grover App')
  })
  ...
  it('returns all matches when multiple items match', () => {
    // Both "Build Grover App" and "React Hooks Deep-Dive" contain 'e'
