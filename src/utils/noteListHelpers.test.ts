...
describe('filterEntries', () => {
  ...
  it('shows only direct root-level files when selecting the vault root folder', () => {
    const entries = [
      makeEntry({ path: '/Users/luca/Grover/root-note.md', title: 'Root Note', fileKind: 'markdown' }),
      makeEntry({ path: '/Users/luca/Grover/config.json', title: 'config.json', fileKind: 'text' }),
      makeEntry({ path: '/Users/luca/Grover/projects/nested.md', title: 'Nested Note', fileKind: 'markdown' }),
      makeEntry({ path: '/Users/luca/Grover/assets/logo.png', title: 'Logo', fileKind: 'binary' }),
    ]

    const result = filterEntries(entries, { kind: 'folder', path: '', rootPath: '/Users/luca/Grover' })
...
describe('buildRelationshipGroups', () => {
  ...
  it('keeps stale entity views usable when a rapidly switched entry lacks filename metadata', () => {
    const staleEntity = {
      ...makeEntry({
        path: 'C:\\Users\\luca\\Grover\\project\\alpha.md',
        title: 'Alpha project',
        isA: 'Project',
      }),
      filename: undefined,
    ...
    const backlink = makeEntry({
      path: '/Users/luca/Grover/note/backlink.md',
