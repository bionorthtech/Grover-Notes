...
describe('useAutoSync', () => {
  ...
  it('pulls on mount (app launch)', async () => {
    renderSync()
    await waitFor(() => {
      expect(mockInvokeFn).toHaveBeenCalledWith('git_pull', { vaultPath: '/Users/luca/Grover' })
    })
  })

  it('pulls all active vaults on mount', async () => {
    renderSync(5, true, ['/Users/luca/Grover', '/Users/luca/Work'])
  ...
  it('dedupes automatic mount, focus, and interval remote-status refreshes within cooldown', async () => {
    ...
    try {
      renderSync(0.001, true, ['/Users/luca/Grover', '/Users/luca/Work'])
  ...
  it('does not call git operations when disabled', async () => {
    ...
    expect(mockInvokeFn).not.toHaveBeenCalledWith('git_pull', { vaultPath: '/Users/luca/Grover' })
    expect(mockInvokeFn).not.toHaveBeenCalledWith('git_remote_status', { vaultPath: '/Users/luca/Grover' })
    ...
    expect(mockInvokeFn).not.toHaveBeenCalledWith('git_pull', { vaultPath: '/Users/luca/Grover' })
    expect(mockInvokeFn).not.toHaveBeenCalledWith('git_push', { vaultPath: '/Users/luca/Grover' })
  })
  ...
  it('calls onVaultUpdated and onToast when pull has updates', async () => {
    ...
    await waitFor(() => {
      expect(onVaultUpdated).toHaveBeenCalledWith(['note.md', 'project/plan.md'], '/Users/luca/Grover')
  ...
  it('waits for vault refresh before showing the updated toast', async () => {
    ...
    await waitFor(() => {
      expect(asyncVaultRefresh).toHaveBeenCalledWith(['note.md'], '/Users/luca/Grover')
    })
  ...
  it('pulls, pushes, and emits a success toast when recovery succeeds', async () => {
    ...
    await waitFor(() => {
      expect(onVaultUpdated).toHaveBeenCalledWith(['note.md'], '/Users/luca/Grover')
  ...
  it('manual triggerSync syncs every active vault by default', async () => {
    const { result } = renderSync(5, true, ['/Users/luca/Grover', '/Users/luca/Work'])
    await waitFor(() => {
      expect(result.current.syncStatus).toBe('idle')
    })
    ...
    await waitFor(() => {
      expect(mockInvokeFn).toHaveBeenCalledWith('git_pull', { vaultPath: '/Users/luca/Grover' })
      expect(mockInvokeFn).toHaveBeenCalledWith('git_pull', { vaultPath: '/Users/luca/Work' })
      expect(onVaultUpdated).toHaveBeenCalledWith(['home.md'], '/Users/luca/Grover')
      expect(onVaultUpdated).toHaveBeenCalledWith(['work.md'], '/Users/luca/Work')
      expect(onToast).toHaveBeenCalledWith('Pulled 2 update(s) from remote')
    })
  })
  ...
  it('pullAndPush pulls and pushes every active vault by default', async () => {
    ...
    await waitFor(() => {
      expect(mockInvokeFn).toHaveBeenCalledWith('git_pull', { vaultPath: '/Users/luca/Grover' })
      expect(mockInvokeFn).toHaveBeenCalledWith('git_pull', { vaultPath: '/Users/luca/Work' })
      expect(mockInvokeFn).toHaveBeenCalledWith('git_push', { vaultPath: '/Users/luca/Grover' })
