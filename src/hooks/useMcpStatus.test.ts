...
describe('useMcpStatus', () => {
  ...
  it.each([
    {
      ...
      toastFragment: 'Grover external AI tools connected successfully',
    },
    {
      action: 'connect' as const,
      commandArgs: { vaultPath: '/vault' },
    ...
    },
    {
      ...
      toastFragment: 'Grover external AI tools disconnected successfully',
    },
    {
      action: 'disconnect' as const,
      commandArgs: undefined,
  ...
  it('loads the exact manual MCP config snippet for the active vault', async () => {
    const snippet = JSON.stringify({ mcpServers: { grover: { type: 'stdio' } } })
    mockCommands({
      check_mcp_status: 'installed',
  ...
  it('copies the manual MCP config snippet to the clipboard', async () => {
    ...
    expect(onToast).toHaveBeenCalledWith('Grover MCP config copied to clipboard')
  })
