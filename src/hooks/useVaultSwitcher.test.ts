...
describe('useVaultSwitcher', () => {
  ...
  it('loads persisted vaults on mount', async () => {
    mockVaultListStore = {
      vaults: [{ label: 'My Vault', path: '/Users/luca/Grover' }],
      active_vault: '/Users/luca/Grover',
    }
    ...
    expect(result.current.allVaults[1].path).toBe('/Users/luca/Grover')
    expect(result.current.allVaults[1].available).toBe(true)
    expect(result.current.vaultPath).toBe('/Users/luca/Grover')
  ...
  it('shows a clear toast when folder picking is blocked until restart', async () => {
    ...
    expect(onToast).toHaveBeenCalledWith(
      'Grover needs a restart before macOS can open another folder picker. Restart to apply the downloaded update and try again.',
    )
  })
