...
describe('useZoom', () => {
  ...
  it('dispatches grover-zoom-change event on zoomIn', () => {
    const handler = vi.fn()
    window.addEventListener('grover-zoom-change', handler)
    const { result } = renderHook(() => useZoom())
    handler.mockClear() // clear any init-phase dispatches
    act(() => result.current.zoomIn())
    expect(handler).toHaveBeenCalled()
    window.removeEventListener('grover-zoom-change', handler)
  })

  it('dispatches grover-zoom-change event on zoomOut', () => {
    const handler = vi.fn()
    window.addEventListener('grover-zoom-change', handler)
    const { result } = renderHook(() => useZoom())
    handler.mockClear()
  ...
  it('dispatches grover-zoom-change event on zoomReset', () => {
    resetVaultConfigStore()
    bindVaultConfigStore({ ...DEFAULT_VC, zoom: 1.2 }, vi.fn())
    const handler = vi.fn()
    window.addEventListener('grover-zoom-change', handler)
