...
describe('useVaultLoader', () => {
  ...
  it('freshly reloads the active mounted workspace on startup in Tauri mode', async () => {
    ...
    const vaults = [grover, brian]
    const groverStartupResponses: Partial<Record<string, VaultEntry[]>> = {
      reload_vault: [
        { ...mockEntries[0], path: '/grover/note/alpha.md', filename: 'alpha.md', title: 'Alpha' },
      ],
      list_vault: [],
    }

    backendInvokeFn.mockImplementation(((cmd: string, args?: Record<string, unknown>) => {
      const response = args?.path === '/grover' ? groverStartupResponses[cmd] : undefined
      if (response) return Promise.resolve(response)
      if (EMPTY_ARRAY_COMMANDS.has(cmd)) return Promise.resolve([])
      return Promise.resolve(null)
    }) as typeof defaultMockInvoke)

    const { result } = renderHook(() => useVaultLoader('/grover', vaults, '/grover', vaults))
    ...
    const groverLoadCommands = backendInvokeFn.mock.calls
      ...
      .map(([command]) => command)
    expect(groverLoadCommands).toContain('reload_vault')
    expect(groverLoadCommands).not.toContain('list_vault')
  })
