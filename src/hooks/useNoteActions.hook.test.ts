...
const makeEntry = (overrides: Partial<VaultEntry> = {}): VaultEntry => ({
  path: '/Users/luca/Grover/test.md',
  filename: 'test.md',
  title: 'Test Note',
  isA: 'Note',
  aliases: [],
...
describe('useNoteActions hook', () => {
  ...
  it('keeps the active tab open when gitignored visibility reports a /tmp alias', async () => {
    const activeEntry = makeEntry({
      path: '/private/tmp/grover-vault/active.md',
      filename: 'active.md',
      title: 'Active',
    })
    ...
    act(() => {
      window.dispatchEvent(new CustomEvent(GITIGNORED_VISIBILITY_APPLIED_EVENT, {
        detail: {
          hide: true,
          visiblePaths: ['/tmp/grover-vault/active.md'],
        },
      }))
    })

    expect(result.current.activeTabPath).toBe('/private/tmp/grover-vault/active.md')
    expect(result.current.tabs).toHaveLength(1)
  })
