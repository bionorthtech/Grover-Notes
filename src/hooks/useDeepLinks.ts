...
function useDeepLinkResolver({
  ...
  const openResolvedDeepLink = useCallback((request: Extract<ResolvedGroverDeepLink, { ok: true }>) => {
  ...
  useEffect(() => {
    ...
    const resolved = resolveGroverDeepLink({ rawUrl: pendingUrl, vaults: knownVaults })
    setPendingUrl(null)
    if (!resolved.ok) {
...
function useDeepLinkTestBridge({
  ...
  useEffect(() => {
    ...
    return () => {
      if (window.__groverTest?.openDeepLink === openDeepLink) {
        delete window.__groverTest.openDeepLink
      }
    }
...
function useDeepLinkCopyActions({
  ...
  const copyEntryDeepLink = useCallback((entry: VaultEntry) => {
    ...
    const result = buildGroverDeepLinkForEntry({ entry, vaultPath, vaults: knownVaults })
    if (!result.ok) {
