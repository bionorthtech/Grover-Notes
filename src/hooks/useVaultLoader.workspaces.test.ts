...
describe('useVaultLoader workspaces', () => {
  ...
  it('keeps the active vault folder root visible when it is absent from mounted folder vaults', async () => {
    const entryVaults = [
      ...
      { label: 'Grover', path: '/grover', alias: 'grover', available: true, mounted: true },
    ]
    const folderVaults = [entryVaults[1]]
    mockWorkspaceBackend({
      foldersByPath: {
        '/brian': [backendFolder('brian-projects')],
        '/grover': [backendFolder('grover-projects')],
      },
    })

    const { result } = renderHook(() => useVaultLoader('/brian', entryVaults, '/grover', folderVaults))

    await waitForEntries(result, 2)
    await waitFor(() => {
      expect(result.current.folders).toEqual([
        mountedFolderRoot('Grover', '/grover', 'grover-projects'),
  ...
  it('reloads scoped folder roots when another mounted workspace is added', async () => {
    ...
    mockWorkspaceBackend({
      foldersByPath: {
        ...
        '/grover': [backendFolder('grover-root')],
        '/third': [backendFolder('third-root')],
      },
    })
    const { result, rerender } = renderHook(
      ({ folderVaults }) => useVaultLoader('/brian', [brian, grover, third], '/brian', folderVaults),
    ...
    rerender({ folderVaults: [brian, grover] })

    await waitFor(() => {
      expect(result.current.folders.map((folder) => folder.rootPath)).toEqual(['/brian', '/grover'])
    })

    rerender({ folderVaults: [brian, grover, third] })

    await waitFor(() => {
      expect(result.current.folders.map((folder) => folder.rootPath)).toEqual(['/brian', '/grover', '/third'])
    })
  })

  it('adds each mounted workspace as soon as that workspace finishes loading', async () => {
    const groverLoad = createDeferred<VaultEntry[]>()
    const brian = { label: 'Brian', path: '/brian', alias: 'brian', available: true, mounted: true }
    const grover = { label: 'Grover', path: '/grover', alias: 'grover', available: true, mounted: true }
    const team = { label: 'Team', path: '/team', alias: 'team', available: true, mounted: true }
    const vaults = [brian, grover, team]

    mockWorkspaceBackend({
      entriesByPath: { '/grover': groverLoad.promise },
    })
    ...
    await act(async () => {
      groverLoad.resolve([{ ...mockEntries[0], path: '/grover/note/hello.md' }])
    })

    await waitFor(() => {
      expect(result.current.entries.map((entry) => entry.workspace?.path).sort()).toEqual(['/brian', '/grover', '/team'])
    })
  })
  ...
  it('uses cached vault listing for background workspace loads in Tauri mode', async () => {
    ...
    const groverLoadCommands = backendInvokeFn.mock.calls
      .filter(([, args]) => args?.path === '/grover')
      .map(([command]) => command)
    expect(groverLoadCommands).toContain('list_vault')
    expect(groverLoadCommands).not.toContain('reload_vault')
  })
