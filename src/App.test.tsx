...
vi.mock('./components/groverEditorFormatting', () => ({
  GroverFormattingToolbar: () => null,
  GroverFormattingToolbarController: () => null,
}))
...
const AI_AGENTS_ONBOARDING_DISMISSED_STORAGE_NAME = 'grover:ai-agents-onboarding-dismissed'
const CLAUDE_CODE_ONBOARDING_DISMISSED_STORAGE_NAME = 'grover:claude-code-onboarding-dismissed'
...
describe('App', () => {
  ...
  it('opens a note window after loading the active vault graph', async () => {
    ...
    await waitFor(() => expect(window.__groverTest?.activeTabPath).toBe('/vault/project/test.md'))
  ...
  it('persists a Cmd+N note before opening it in the editor', async () => {
    ...
    try {
      ...
      expect(window.__groverTest?.activeTabPath).not.toBe('/vault/untitled-note-1700000000.md')
      ...
      await waitFor(() => {
        expect(window.__groverTest?.activeTabPath).toBe('/vault/untitled-note-1700000000.md')
      })
  ...
  it('shows visible feedback when a manual update check finds an update', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Grover 2026.4.25 is available')).toBeInTheDocument()
    })
  })
  ...
  it('shows visible feedback when a menu-driven update check finds no eligible update', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('All Notes')).toBeInTheDocument()
      expect(typeof window.__groverTest?.dispatchBrowserMenuCommand).toBe('function')
    })

    act(() => {
      window.__groverTest?.dispatchBrowserMenuCommand?.('app-check-for-updates')
    })
  ...
  it('shows the external AI setup dialog from the menu when AI onboarding is active', async () => {
    ...
    act(() => {
      window.__groverTest?.dispatchBrowserMenuCommand?.('vault-install-mcp')
    })
  ...
  it('shows onboarding after telemetry consent when no active vault is configured', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Help improve Grover')).toBeInTheDocument()
    }, { timeout: SLOW_APP_READY_TIMEOUT_MS })
  ...
  it.each([
    ...
    localStorage.setItem('grover_welcome_dismissed', '1')
  ...
  it('shows welcome instead of vault-missing when the missing path was not a persisted active vault', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Welcome to Grover')).toBeInTheDocument()
    })
    expect(screen.queryByText('Vault not found')).not.toBeInTheDocument()
    expect(screen.getByTestId('welcome-open-folder')).toHaveTextContent('Open existing vault')
  })
  ...
  it('defaults to All Notes when explicit organization is disabled in vault config', async () => {
    ...
    localStorage.setItem(`grover:vault-config:${workVaultPath}`, disabledWorkflowConfig)
  ...
  it('auto-advances to the next inbox item after organizing when the setting is enabled', async () => {
    ...
    await waitFor(() => {
      expect(window.__groverTest?.activeTabPath).toBe('/vault/beta.md')
    })
  }, 10_000)
  ...
  it('keeps the manually selected note after organizing finishes later', async () => {
    ...
    await waitFor(() => {
      expect(window.__groverTest?.activeTabPath).toBe('/vault/gamma.md')
    })
