...
import { GROVER_DOCS_URL } from './constants/feedback'
...
function MainApp({ noteWindowParams }: { noteWindowParams: NoteWindowParams | null }) {
  ...
  const openDocs = useCallback(() => {
    void openExternalUrl(GROVER_DOCS_URL)
  }, [])
  ...
  useEffect(() => {
    ...
    return () => {
      if (window.__groverTest?.seedAutoGitSavedChange === seedAutoGitSavedChange) {
        delete window.__groverTest.seedAutoGitSavedChange
      }
    }
  ...
  const handleCheckForUpdates = useCallback(async () => {
    ...
    if (result.kind === 'up-to-date') {
      ...
      setToastMessage(`Grover ${result.displayVersion} is available`)
    } else {
  ...
  const restoreVaultAiGuidance = useCallback(async (successToast: string | null = 'Grover AI guidance restored') => {
    if (!resolvedPath) return
    try {
      const tauriInvoke = isTauri() ? invoke : mockInvoke
      ...
      setToastMessage(`Failed to restore Grover AI guidance: ${err}`)
    }
  }, [refreshVaultAiGuidance, resolvedPath, vault])
  ...
  const commands = useAppCommands({
    ...
    onShowTasks: () => { trackEvent('insights_opened', { tool: 'tasks' }); void vaultTasks.requestTasks() },
    onQueryNotes: () => { trackEvent('insights_opened', { tool: 'query' }); setQueryDialogOpen(true) },
    onVaultHealth: () => { trackEvent('insights_opened', { tool: 'health' }); setHealthDialogOpen(true) },
    onVaultStats: () => { trackEvent('insights_opened', { tool: 'stats' }); setStatsDialogOpen(true) },
