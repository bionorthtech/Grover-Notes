...
vi.mock('./groverEditorFormattingConfig', () => ({
  getGroverSlashMenuItems: vi.fn(async () => []),
}))

vi.mock('./groverEditorFormatting', () => ({
  GroverFormattingToolbar: () => <div data-testid="grover-formatting-toolbar" />,
  GroverFormattingToolbarController: (props: Record<string, unknown>) => {
    state.capturedToolbarProps = props
    return <div data-testid="grover-formatting-toolbar-controller" />
  },
}))

vi.mock('./groverBlockNoteSideMenu', () => ({
  GroverSideMenu: () => <div data-testid="grover-side-menu" />,
}))
...
describe('SingleEditorView', () => {
  ...
  it('registers the seeded BlockNote test bridge, applies column widths, and cleans it up on unmount', async () => {
    ...
    expect(typeof window.__groverTest?.seedBlockNoteTable).toBe('function')

    await act(async () => {
      await window.__groverTest?.seedBlockNoteTable?.([120, null, 80])
    })
    ...
    expect(window.__groverTest?.seedBlockNoteTable).toBeUndefined()
  })
