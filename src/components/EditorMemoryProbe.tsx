...
declare global {
  interface Window {
    __groverEditorMemoryProbe?: EditorMemoryProbeApi
  }
}
...
function useEditorMemoryProbeBridge(api: EditorMemoryProbeApi): void {
  useEffect(() => {
    ...
    window.__groverEditorMemoryProbe = api
    return () => {
      if (window.__groverEditorMemoryProbe?.run === api.run) {
        delete window.__groverEditorMemoryProbe
      }
    }
  }, [api])
}
