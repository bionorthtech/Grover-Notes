...
import {
  ...
  GROVER_GITHUB_CONTRIBUTING_URL,
  GROVER_GITHUB_DISCUSSIONS_URL,
  GROVER_GITHUB_ISSUES_URL,
  GROVER_GITHUB_PULL_REQUESTS_URL,
  GROVER_PRODUCT_BOARD_URL,
...
describe('FeedbackDialog', () => {
  ...
  it('renders the contribution paths when open', () => {
    ...
    expect(screen.getByText('Contribute to Grover')).toBeInTheDocument()
    ...
    expect(screen.getByText(/Grover is FOSS and always will be/i)).toBeInTheDocument()
  ...
  it('opens the expected contribution links without closing the modal', async () => {
    ...
    expect(openExternalUrl).toHaveBeenNthCalledWith(2, GROVER_PRODUCT_BOARD_URL)
    expect(openExternalUrl).toHaveBeenNthCalledWith(3, GROVER_GITHUB_DISCUSSIONS_URL)
    expect(openExternalUrl).toHaveBeenNthCalledWith(4, GROVER_GITHUB_PULL_REQUESTS_URL)
    expect(openExternalUrl).toHaveBeenNthCalledWith(5, GROVER_GITHUB_CONTRIBUTING_URL)
    expect(openExternalUrl).toHaveBeenNthCalledWith(6, GROVER_GITHUB_ISSUES_URL)
  ...
  it('copies a sanitized diagnostic bundle', async () => {
    ...
    expect(writeText.mock.calls[0]?.[0]).toContain('Grover sanitized diagnostics')
  ...
  it('shows a fallback message when a contribution link cannot be opened', async () => {
    ...
    expect(screen.getByText(GROVER_PRODUCT_BOARD_URL)).toBeInTheDocument()
  })
