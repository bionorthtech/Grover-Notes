...
const GROVER_TLDRAW_USER_ID = 'grover-whiteboard'
...
function installZoomAwareViewport(editor: Editor): () => void {
  ...
  window.addEventListener('grover-zoom-change', scheduleViewportUpdate)

  return () => {
    window.removeEventListener('grover-zoom-change', scheduleViewportUpdate)
...
interface GroverTldrawDialogProps {
  dialog: TLUiDialog
  onClose: (id: string) => void
}

const DIALOG_OPEN_DISMISS_GRACE_MS = 250
let retainedGroverTldrawDialogs: TLUiDialog[] = []
...
interface GroverTldrawDialogContentProps {
  dialog: TLUiDialog
  mouseDownInsideContentRef: MutableRefObject<boolean>
  onClose: () => void
}

function GroverTldrawDialogContent({
  dialog,
  mouseDownInsideContentRef,
  onClose,
}: GroverTldrawDialogContentProps) {
...
const GroverTldrawDialog = memo(function GroverTldrawDialog({ dialog, onClose }: GroverTldrawDialogProps) {
...
function GroverTldrawDialogs() {
  const { dialogs, removeDialog } = useDialogs()
  const requestedDialogs = useValue('grover tldraw dialogs', () => dialogs.get(), [dialogs])
  const [visibleDialogs, setVisibleDialogs] = useState<TLUiDialog[]>(() =>
    retainedGroverTldrawDialogs.length > 0 ? retainedGroverTldrawDialogs : dialogs.get()
  )

  const closeVisibleDialog = useCallback((id: string) => {
    const nextDialogs = retainedGroverTldrawDialogs.filter((dialog) => dialog.id !== id)
    retainedGroverTldrawDialogs = nextDialogs
  ...
  useEffect(() => {
    ...
    retainedGroverTldrawDialogs = requestedDialogs
    queueMicrotask(() => {
...
export function TldrawWhiteboard({
  ...
  const tldrawUiComponents = useMemo(() => ({ Dialogs: GroverTldrawDialogs }), [])
