...
function changeFile(
  ...
  return {
    path: entry.path,
    relativePath: entry.path.replace('/Users/luca/Grover/', ''),
    status,
    ...overrides,
  }
}
...
describe('NoteList changes view', () => {
  it('shows only modified notes in changes view with note titles and filenames', () => {
    ...
    expect(screen.getByText('Build Grover App')).toBeInTheDocument()
    expect(screen.getByText('Facebook Ads Strategy')).toBeInTheDocument()
    expect(screen.getByText('26q1-grover-app.md')).toBeInTheDocument()
  ...
  it('matches entries by relative path suffix across machines', () => {
    const crossMachineEntries: VaultEntry[] = mockEntries.map((entry) => ({
      ...entry,
      path: entry.path.replace('/Users/luca/Grover', '/Users/other-machine/OtherVault'),
    }))
  ...
  it('shows deleted notes as rows when files are deleted', () => {
    const filesWithDeleted = [
      changeFile(mockEntries[0], 'modified', { addedLines: 42, deletedLines: 7 }),
      { path: '/Users/luca/Grover/note/gone.md', relativePath: 'note/gone.md', status: 'deleted' as const, deletedLines: 5 },
      { path: '/Users/luca/Grover/note/also-gone.md', relativePath: 'note/also-gone.md', status: 'deleted' as const, deletedLines: 2 },
    ]
  ...
  it('shows the discard context menu when onDiscardFile is provided', () => {
    ...
    const noteItem = screen.getByText('Build Grover App').closest('[class*="border-b"]')!
  ...
  it('shows the confirmation dialog after clicking discard changes', () => {
    ...
    expect(dialog.textContent).toContain('Build Grover App')
  })
  ...
  it('calls onDiscardFile with the relative path when discard is confirmed', async () => {
    ...
    expect(onDiscardFile).toHaveBeenCalledWith('project/26q1-grover-app.md')
  })
