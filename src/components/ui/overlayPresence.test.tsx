...
describe('overlay presence stability', () => {
  beforeEach(() => {
    document.documentElement.style.removeProperty('--grover-overlay-zoom-factor')
    document.documentElement.style.removeProperty('--grover-overlay-zoom-inverse')
  })
  ...
  it('publishes zoom variables for overlay portal positioning and visual scale', () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--grover-overlay-zoom-factor')).toBe(String(110 / 100))
    expect(document.documentElement.style.getPropertyValue('--grover-overlay-zoom-inverse')).toBe(String(100 / 110))
  })

  it('compensates tooltip portal positioning without cancelling content zoom', () => {
    document.documentElement.style.setProperty('--grover-overlay-zoom-factor', '1.4')
    document.documentElement.style.setProperty('--grover-overlay-zoom-inverse', String(1 / 1.4))
    ...
    expect(positionShell.className).toContain('[zoom:var(--grover-overlay-zoom-inverse,1)]')
    expect(visualShell.className).toContain('[zoom:var(--grover-overlay-zoom-factor,1)]')
  })
