...
const mockFolders: FolderNode[] = [
  {
    ...
    path: 'projects',
    children: [
      { name: 'grover', path: 'projects/grover', children: [] },
...
const vaultRootPath = '/Users/luca/Grover'
...
describe('FolderTree', () => {
  ...
  it('renders the vault root as the top-level folder when a vault path is available', () => {
    ...
    expect(screen.getByRole('button', { name: 'Grover' })).toBeInTheDocument()
    expect(screen.getByText('projects')).toBeInTheDocument()
    expect(screen.getByText('areas')).toBeInTheDocument()
    expect(screen.getByText('journal')).toBeInTheDocument()
  })
  ...
  it('expands children when clicking a folder row', () => {
    vi.useFakeTimers()
    render(<FolderTree folders={mockFolders} selection={defaultSelection} onSelect={vi.fn()} />)
    expect(screen.queryByText('grover')).not.toBeInTheDocument()
  ...
  it('passes the selected folder as the parent when creating a new folder', async () => {
    ...
    await submitNewFolder('grover')

    await vi.waitFor(() => {
      expect(onCreateFolder).toHaveBeenCalledWith('grover', {
        path: 'projects',
  ...
  it('keeps folder toggling healthy after cancelling rename', () => {
    ...
    const wasExpanded = screen.queryByText('grover') !== null
    fireEvent.click(screen.getByTestId('folder-row:projects'))
    act(() => {
      vi.advanceTimersByTime(FOLDER_ROW_SINGLE_CLICK_DELAY_MS)
    })

    expect(screen.queryByText('grover') !== null).toBe(!wasExpanded)
    vi.useRealTimers()
  })
  ...
  it('keeps destructive folder actions off the vault root row and menu', () => {
    ...
    fireEvent.contextMenu(screen.getByText('Grover'))
