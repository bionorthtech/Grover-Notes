...
type GroverSlashMenuItem = DefaultReactSuggestionItem & { key: string }
type GroverBlockTypeSelectItem = {
  name: string
  type: string
...
const GROVER_BLOCK_TYPE_SELECT_ITEMS: GroverBlockTypeSelectItem[] = [
...
const GROVER_SLASH_MENU_ICONS: Partial<Record<string, PhosphorIcon>> = {
  audio: SpeakerHigh,
...
export function createMathSlashMenuItem(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  labels: GroverSlashMenuLabels = { mathTitle: 'Math' },
): GroverSlashMenuItem {
  return createBlockSlashMenuItem(editor, {
    key: 'math',
...
function createBlockSlashMenuItem(
  ...
  } as GroverSlashMenuItem
}

export function addItemsToMediaGroup(
  items: GroverSlashMenuItem[],
  mediaItems: GroverSlashMenuItem[],
): GroverSlashMenuItem[] {
...
function createGroverSlashMenuIcon(Icon: PhosphorIcon) {
  return createElement(
    'span',
    { className: 'grover-slash-menu-icon' },
    createElement(Icon, {
      'aria-hidden': true,
      className: 'grover-slash-menu-icon__regular',
      size: 18,
      weight: 'regular',
    }),
    createElement(Icon, {
      'aria-hidden': true,
      className: 'grover-slash-menu-icon__fill',
      size: 18,
      weight: 'fill',
    }),
  )
...
export function getGroverBlockTypeSelectItems() {
  return GROVER_BLOCK_TYPE_SELECT_ITEMS
}

export function filterGroverFormattingToolbarItems<T extends ReactElement>(
  items: T[],
): T[] {
  return items.filter(
...
export function filterGroverSlashMenuItems<T extends GroverSlashMenuItem>(
  items: T[],
): T[] {
  return items
    .filter((item) => !UNSUPPORTED_SLASH_MENU_KEYS.has(item.key))
    .map((item) => {
      const GroverIcon = GROVER_SLASH_MENU_ICONS[item.key]

      return {
        ...item,
        icon: GroverIcon ? createGroverSlashMenuIcon(GroverIcon) : item.icon,
        subtext: undefined,
      }
    }) as T[]
}

export function getGroverSlashMenuItems(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  query: string,
  labels?: GroverSlashMenuLabels,
) {
  const items = addItemsToMediaGroup(
    getDefaultReactSlashMenuItems(editor) as GroverSlashMenuItem[],
    [
