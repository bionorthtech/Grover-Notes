...
import {
  filterGroverFormattingToolbarItems,
  getGroverBlockTypeSelectItems,
} from './groverEditorFormattingConfig'
...
type GroverBasicTextStyle = 'bold' | 'italic' | 'strike' | 'code'

const FORMATTER_CLOSE_GRACE_MS = 160
const FORMATTER_VIEWPORT_PADDING_PX = 8
type GroverFloatingOptions = NonNullable<FloatingUIOptions['useFloatingOptions']>
type GroverFloatingMiddleware = NonNullable<GroverFloatingOptions['middleware']>[number]
...
const GROVER_BASIC_TEXT_STYLE_TOOLTIPS = {
  bold: {
    label: 'Bold',
  ...
  GroverBasicTextStyle,
  { label: string; mainTooltip: string; secondaryTooltip: string }
>

const GROVER_BASIC_TEXT_STYLE_ICONS = {
  bold: Bold,
  italic: Italic,
  strike: Strikethrough,
  code: Code2,
} satisfies Record<GroverBasicTextStyle, PhosphorIcon>

type GroverSelectedBlock = ReturnType<
  BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>['getTextCursorPosition']
>['block']

type GroverSelectedFileBlock = {
  type: string
  url: string
}
...
type GroverBlockTypeSelectOption = ReturnType<
  typeof getGroverBlockTypeSelectItems
>[number] & {
...
function viewportClampMiddleware(): GroverFloatingMiddleware {
  return {
    name: 'groverViewportClamp',
...
function withViewportSafeMiddleware(
  options?: GroverFloatingOptions,
): GroverFloatingOptions {
  if (!options) {
    return {
...
function getSelectedBlocksSafely(
  ...
  try {
    ...
    if (selectionBlocks?.length) {
      return selectionBlocks as GroverSelectedBlock[]
    }
  } catch {
  ...
  try {
    return [editor.getTextCursorPosition().block as GroverSelectedBlock]
  } catch {
    return []
  }
}
...
function getBasicTextStyleButtonState(
  basicTextStyle: GroverBasicTextStyle,
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
) {
...
function getBlockTypeItemIconElement(
  item: ReturnType<typeof getGroverBlockTypeSelectItems>[number],
) {
...
function isSelectedBlockTypeItem(
  item: ReturnType<typeof getGroverBlockTypeSelectItems>[number],
  firstSelectedBlock: GroverSelectedBlock,
) {
...
function getGroverBlockTypeSelectOptions(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  firstSelectedBlock: GroverSelectedBlock,
) {
  return getGroverBlockTypeSelectItems()
    .filter((item) =>
      editorHasBlockWithType(
        editor,
        item.type,
...
function liveSelectedBlock(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  block: GroverSelectedBlock,
) {
  try {
    return editor.getBlock(block.id) as GroverSelectedBlock | undefined
  } catch (error) {
...
function liveSelectedBlocks(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  selectedBlocks: GroverSelectedBlock[],
) {
  const liveBlocks: GroverSelectedBlock[] = []
...
function GroverBasicTextStyleButton({
  basicTextStyle,
}: {
  basicTextStyle: GroverBasicTextStyle
}) {
  ...
  const Icon = Reflect.get(GROVER_BASIC_TEXT_STYLE_ICONS, basicTextStyle) as PhosphorIcon
  const copy = Reflect.get(GROVER_BASIC_TEXT_STYLE_TOOLTIPS, basicTextStyle) as {
    label: string
    mainTooltip: string
...
function GroverBlockTypeSelect() {
  const editor = useBlockNoteEditor<
    BlockSchema,
    InlineContentSchema,
    StyleSchema
  ...
  const selectedBlocks = useEditorState({
    editor,
    selector: ({ editor }): GroverSelectedBlock[] => getSelectedBlocksSafely(editor),
  })
  const firstSelectedBlock = selectedBlocks[0] ?? null
  const selectItems = useMemo(
    () => (
      firstSelectedBlock
        ? getGroverBlockTypeSelectOptions(editor, firstSelectedBlock)
        : []
    ),
    [editor, firstSelectedBlock],
  )
  const selectedItem = selectItems.find(
    (item): item is GroverBlockTypeSelectOption => item.isSelected,
  )
...
function GroverFileDownloadButton({ vaultPath }: { vaultPath?: string }) {
  const Components = useComponentsContext()!
  ...
  return items.flatMap((item) => {
    switch (String(item.key)) {
      case 'blockTypeSelect':
        return [<GroverBlockTypeSelect key={item.key} />]
      case 'boldStyleButton':
        return [<GroverBasicTextStyleButton basicTextStyle="bold" key={item.key} />]
      case 'italicStyleButton':
        return [<GroverBasicTextStyleButton basicTextStyle="italic" key={item.key} />]
      case 'strikeStyleButton':
        return [<GroverBasicTextStyleButton basicTextStyle="strike" key={item.key} />]
      case 'fileDownloadButton':
        return [<GroverFileDownloadButton key={item.key} vaultPath={vaultPath} />]
      default:
        return [item]
    }
  })
...
function insertInlineCodeButton(items: ReactElement[]) {
  ...
  return [
    ...
    <GroverBasicTextStyleButton basicTextStyle="code" key="codeStyleButton" />,
    ...items.slice(strikeButtonIndex + 1),
  ]
}

function getGroverFormattingToolbarItems(vaultPath?: string) {
...
export function GroverFormattingToolbar({ vaultPath }: { vaultPath?: string } = {}) {
  return <FormattingToolbar>{getGroverFormattingToolbarItems(vaultPath)}</FormattingToolbar>
}

export function GroverFormattingToolbarController(props: {
  ...
  const Component = props.formattingToolbar || GroverFormattingToolbar
