...
type GroverBlockNoteEditor = BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>
type GroverBlock = NonNullable<ReturnType<GroverBlockNoteEditor['getBlock']>>
type SideMenuBlock = {
  content?: unknown
  id: string
  type: string
...
function liveSideMenuBlock(editor: GroverBlockNoteEditor, block: SideMenuBlock | undefined) {
...
function hasChildBlock(block: GroverBlock, blockId: string): boolean {
...
function editorBlockElement(editor: GroverBlockNoteEditor): HTMLElement | null {
...
function createSideMenuAlignmentController(editor: GroverBlockNoteEditor, blockId: string) {
...
function useSideMenuTextAlignment(editor: GroverBlockNoteEditor, block: SideMenuBlock | undefined) {
...
function createDropIndicator(ownerDocument: Document): HTMLElement {
  ...
  indicator.style.background = 'var(--border-focus, #0a5a38)'
...
function GroverDragHandleButton({
  children,
  dragHandleMenu,
  ...
  return (
    ...
    >
      <Components.Generic.Menu.Trigger>
        <span
          className="grover-block-drag-handle"
...
function GroverRemoveBlockItem({ children }: { children: ReactNode }) {
  const Components = useComponentsContext()!
  const { block, editor } = useSideMenuBlock()
...
function GroverTableHeaderItem({
  children,
  header,
}: {
  children: ReactNode
...
function GroverDragHandleMenu() {
  const dict = useDictionary()

  return (
    <DragHandleMenu>
      <GroverRemoveBlockItem>{dict.drag_handle.delete_menuitem}</GroverRemoveBlockItem>
      <GroverTableHeaderItem header="row">{dict.drag_handle.header_row_menuitem}</GroverTableHeaderItem>
      <GroverTableHeaderItem header="column">{dict.drag_handle.header_column_menuitem}</GroverTableHeaderItem>
    </DragHandleMenu>
  )
}

export function GroverSideMenu(props: SideMenuProps) {
  const { block, editor } = useSideMenuBlock()
  useSideMenuTextAlignment(editor, block)

  return (
    <SideMenu {...props}>
      <GroverAddBlockButton />
      <GroverDragHandleButton dragHandleMenu={GroverDragHandleMenu} />
    </SideMenu>
  )
}
