...
`
  ...
  it('infers the current workspace for untagged default-vault notes before moving them', () => {
    const groverWorkspace = makeWorkspace('/Users/luca/Grover', 'Grover')
    const refactoringWorkspace = makeWorkspace('/Users/luca/Refactoring', 'Refactoring')
    const entry = {
      ...mockEntry,
      path: '/Users/luca/Grover/project/test.md',
      workspace: undefined,
    }
    ...
    renderSelectedInspector({
      entry,
      workspaces: [groverWorkspace, refactoringWorkspace],
      onChangeWorkspace,
    })

    fireEvent.pointerDown(screen.getByRole('combobox', { name: 'Grover' }), { button: 0, pointerType: 'mouse' })
    fireEvent.click(screen.getByRole('option', { name: 'Refactoring' }))

    expect(onChangeWorkspace).toHaveBeenCalledWith(
      expect.objectContaining({ path: entry.path, workspace: groverWorkspace }),
...
`
  ...
  describe('Referenced By (bidirectional relationships)', () => {
    const targetEntry: VaultEntry = {
      path: '/Users/luca/Grover/responsibility/grow-newsletter.md',
    ...
    const essayEntry: VaultEntry = {
      path: '/Users/luca/Grover/essay/on-writing.md',
    ...
    const procedureEntry: VaultEntry = {
      path: '/Users/luca/Grover/procedure/write-essays.md',
    ...
    const experimentEntry: VaultEntry = {
      path: '/Users/luca/Grover/experiment/seo.md',
      filename: 'seo.md',
...
`
    ...
    it('skips Type relationships in referenced-by computation', () => {
      const typeEntry: VaultEntry = {
        ...targetEntry,
        path: '/Users/luca/Grover/responsibility.md',
