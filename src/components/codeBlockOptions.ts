...
type GroverCodeHighlighter = Awaited<ReturnType<NonNullable<typeof codeBlockOptions.createHighlighter>>>
type GroverLoadLanguage = GroverCodeHighlighter['loadLanguage']
type GroverLanguageInput = Parameters<GroverLoadLanguage>[number]
type GroverLanguageLoader = () => Promise<GroverLanguageInput[]>
type GroverNamedLanguageRegistration = Record<string, unknown> & {
  name: string
  displayName?: string
  aliases?: string[]
}
...
function languageInputs(languages: readonly GroverLanguageInput[]): GroverLanguageInput[] {
  return [...languages]
}

function namedLanguageRegistration(value: GroverLanguageInput): GroverNamedLanguageRegistration | null {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return null
  const record = value as Record<string, unknown>
  return typeof record.name === 'string'
    ? record as GroverNamedLanguageRegistration
    : null
}

function renameLanguageRegistration(
  languages: readonly GroverLanguageInput[],
  sourceName: string,
  ...
  return languages.map((language) => {
    ...
    return { ...registration, ...nextLanguage } as GroverLanguageInput
  })
}

async function loadVbScriptLanguage(): Promise<GroverLanguageInput[]> {
...
const EXTRA_LANGUAGE_LOADERS = new Map<string, GroverLanguageLoader>([
...
function expandGoLanguage(language: string): GroverLanguageInput[] | null {
  return canonicalKnownCodeBlockLanguage(language) === 'go'
    ? [GO_LANGUAGE_REGISTRATION as GroverLanguageInput]
    : null
}

async function expandExternalLanguage(language: string): Promise<GroverLanguageInput[] | null> {
...
async function expandLanguage(language: GroverLanguageInput): Promise<GroverLanguageInput[]> {
  if (typeof language !== 'string') return [language]
  return expandGoLanguage(language) ?? await expandExternalLanguage(language) ?? [language]
}

async function createGroverCodeHighlighter(): Promise<GroverCodeHighlighter> {
...
export function createGroverCodeBlockOptions(): Partial<CodeBlockOptions> {
  const options: Partial<CodeBlockOptions> = {
    ...codeBlockOptions,
    createHighlighter: createGroverCodeHighlighter,
