...
import { getGroverSlashMenuItems } from './groverEditorFormattingConfig'
import {
  GroverFormattingToolbar,
  GroverFormattingToolbarController,
} from './groverEditorFormatting'
import { GroverSideMenu } from './groverBlockNoteSideMenu'
...
function GroverLinkToolbar({ vaultPath, ...props }: LinkToolbarProps & { vaultPath?: string }) {
  return (
    <LinkToolbar {...props}>
      <EditLinkButton
        url={props.url}
      ...
      />
      <GroverOpenLinkButton url={props.url} vaultPath={vaultPath} />
      <DeleteLinkButton
        range={props.range}
        setToolbarOpen={props.setToolbarOpen}
      />
...
function useSeedBlockNoteTableBridge(editor: ReturnType<typeof useCreateBlockNote>) {
  useEffect(() => {
    ...
    return () => {
      if (window.__groverTest?.seedBlockNoteTable === seedBlockNoteTable) {
        delete window.__groverTest.seedBlockNoteTable
      }
    }
  }, [editor])
}
...
function useSuggestionMenuItems(options: {
  ...
  const getSlashMenuItems = useCallback(async (query: string) => {
    try {
      return guardSuggestionMenuItems(
        await Promise.resolve(getGroverSlashMenuItems(editor, query, {
...
function EditorInteractionControllers({
  ...
  return (
    <>
      <SideMenuController sideMenu={GroverSideMenu} />
      <GroverFormattingToolbarController
        formattingToolbar={(props) => (
          <GroverFormattingToolbar {...props} vaultPath={vaultPath} />
        )}
        floatingUIOptions={{
          elementProps: {
      ...
      <LinkToolbarController
        linkToolbar={(props) => (
          <GroverLinkToolbar {...props} vaultPath={vaultPath} />
        )}
        floatingUIOptions={{
          elementProps: {
            onMouseDownCapture: handleToolbarMouseDownCapture,
