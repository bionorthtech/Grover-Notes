...
describe('groverEditorFormatting', () => {
  it('keeps the markdown-safe toolbar controls and block type select', () => {
    const itemKeys = filterGroverFormattingToolbarItems(
      getFormattingToolbarItems(getGroverBlockTypeSelectItems()),
  ...
  it('returns the audited markdown-safe block types for the toolbar select', () => {
    expect(getGroverBlockTypeSelectItems()).toEqual([
  ...
  it('filters unsupported toggle slash-menu variants and removes command descriptions', () => {
    ...
    const items = filterGroverSlashMenuItems([
    ...
    ] satisfies GroverSlashMenuTestItem[])
  ...
  it('wraps slash-menu icons so hover can swap Phosphor weights', () => {
    ...
    >
    expect(icon.props.className).toBe('grover-slash-menu-icon')
    expect(iconChildren.map((child) => child.props.className)).toEqual([
      'grover-slash-menu-icon__regular',
      'grover-slash-menu-icon__fill',
    ])
