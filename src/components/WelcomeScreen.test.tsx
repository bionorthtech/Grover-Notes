...
import groverIcon from '@/assets/grover-icon.svg'
...
const defaultProps = {
  ...
  defaultVaultPath: '~/Documents/Grover',
  onCreateVault: vi.fn(),
...
describe('WelcomeScreen', () => {
  ...
  describe('welcome mode', () => {
    ...
    it('renders the local Grover branding icon', () => {
      render(<WelcomeScreen {...defaultProps} />)

      const brandIcon = screen.getByAltText('Grover icon')
      expect(brandIcon).toHaveAttribute('src', groverIcon)
    })
    ...
    it('shows the simplified template option description', () => {
      ...
      expect(screen.queryByText(/~\/Documents\/Grover/)).not.toBeInTheDocument()
    })
