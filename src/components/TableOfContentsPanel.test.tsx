...
describe('TableOfContentsPanel', () => {
  ...
  it('does not duplicate the note title when the first markdown H1 matches it', () => {
    ...
    expect(toc.title).toBe('Introducing Grover')
    expect(toc.children.map((item) => item.title)).toEqual(['Grover + Refactoring', 'Principles'])
  })
  ...
  it('keeps navigation ids after removing a duplicate markdown title H1', async () => {
    ...
    render(
      <TableOfContentsPanel
        editor={{
          document: [
            { id: 'title-block', type: 'heading', props: { level: 1 }, content: [{ type: 'text', text: 'Introducing Grover' }] },
            { id: 'section-block', type: 'heading', props: { level: 2 }, content: [{ type: 'text', text: 'Grover + Refactoring' }] },
          ],
          setTextCursorPosition,
        }}
        entry={{ ...entry, title: 'Introducing Grover' } as VaultEntry}
        sourceContent={'# Introducing Grover\n\n## Grover + Refactoring'}
        onClose={vi.fn()}
      />,
    )

    fireEvent.click(await screen.findByRole('button', { name: /Introducing Grover/ }))
    expect(setTextCursorPosition).toHaveBeenCalledWith('title-block', 'start')

    fireEvent.click(await screen.findByRole('button', { name: /Grover \+ Refactoring/ }))
