...
describe('telemetry scrubPaths', () => {
  it('redacts macOS absolute paths', () => {
    expect(scrubPaths('Error in /Users/luca/Grover/note.md')).toBe(
...
describe('initSentry', () => {
  it.each([
    ...
    expect(sentryMocks.setTag).toHaveBeenCalledWith('grover.build_version', buildVersion)
    expect(sentryMocks.setTag).toHaveBeenCalledWith('grover.release_kind', releaseKind)
  })
})
