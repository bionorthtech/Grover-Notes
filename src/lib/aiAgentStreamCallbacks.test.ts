...
describe('aiAgentStreamCallbacks', () => {
  ...
  it('repairs missing sentence boundaries between streamed text chunks', () => {
    ...
    callbacks.onText('Created [[Grover Mobile]] as a Project note with a relation to [[frontend]].')
    callbacks.onText('It covers three tech stack paths.')
    callbacks.onDone()

    expect(messages.getMessages()[0].response).toBe(
      "I'll create the Project note now. Created [[Grover Mobile]] as a Project note with a relation to [[frontend]]. It covers three tech stack paths.",
    )
  })
