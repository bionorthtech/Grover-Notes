...
function mockModifiedFiles(): ModifiedFile[] {
  return [
    { path: '/Users/luca/Grover/26q1-grover-app.md', relativePath: '26q1-grover-app.md', status: 'modified' },
    { path: '/Users/luca/Grover/facebook-ads-strategy.md', relativePath: 'facebook-ads-strategy.md', status: 'modified' },
    { path: '/Users/luca/Grover/ai-agents-primer.md', relativePath: 'ai-agents-primer.md', status: 'added' },
    { path: '/Users/luca/Grover/old-draft.md', relativePath: 'old-draft.md', status: 'deleted' },
  ]
}
...
export const mockHandlers: Record<string, (args: any) => any> = {
  ...
  get_modified_files: () => {
    ...
    const extra: ModifiedFile[] = [...mockSavedSinceCommit]
      ...
      .map(p => ({ path: p, relativePath: p.replace(/^.*?\/Grover\//, ''), status: 'modified' as const }))
    return [...base, ...extra]
  },
  ...
  get_last_commit_info: (): LastCommitInfo => ({ shortHash: 'a1b2c3d', commitUrl: 'https://github.com/lucaong/grover-vault/commit/a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0' }),
  is_git_repo: () => true,
  init_git_repo: () => null,
  ...
  get_vault_pulse: (args: { limit?: number }): PulseCommit[] => {
    ...
    const commits: PulseCommit[] = [
      { hash: 'a1b2c3d4e5f6', shortHash: 'a1b2c3d', message: 'Update project notes and add new experiment', date: ts - 3600, githubUrl: 'https://github.com/lucaong/grover-vault/commit/a1b2c3d4e5f6', files: [{ path: '26q1-grover-app.md', status: 'modified', title: '26q1 grover app' }, { path: 'ai-search.md', status: 'added', title: 'ai search' }], added: 1, modified: 1, deleted: 0 },
      { hash: 'b2c3d4e5f6g7', shortHash: 'b2c3d4e', message: 'Reorganize people notes', date: ts - 86400, githubUrl: 'https://github.com/lucaong/grover-vault/commit/b2c3d4e5f6g7', files: [{ path: 'alice-johnson.md', status: 'modified', title: 'alice johnson' }, { path: 'bob-smith.md', status: 'modified', title: 'bob smith' }, { path: 'old-contact.md', status: 'deleted', title: 'old contact' }], added: 0, modified: 2, deleted: 1 },
  ...
  save_image: (args: { vault_path?: string; filename: string; data: string }) => {
    const vault = args.vault_path ?? '/Users/luca/Grover'
  ...
  get_mcp_config_snippet: () => JSON.stringify({
    mcpServers: {
      grover: {
        type: 'stdio',
        command: 'node',
        args: ['/mock/Grover/mcp-server/index.js'],
        env: {
          WS_UI_PORT: '9711',
        },
      },
