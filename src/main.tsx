...
window.__groverTest = {
  ...
  async triggerMenuCommand(id: string) {
    ...
    if (!window.__groverTest?.dispatchBrowserMenuCommand) {
      throw new Error('Grover test bridge is missing dispatchBrowserMenuCommand')
    }

    window.__groverTest.dispatchBrowserMenuCommand(id)
    return undefined
  },
...
function showFatalRenderError(
  ...
  const existing = document.getElementById('grover-fatal-render-error')
  const overlay = existing ?? document.createElement('pre')
  overlay.id = 'grover-fatal-render-error'
