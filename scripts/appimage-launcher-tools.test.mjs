...
test('fixed resolver follows absolute and relative symlinks before choosing AppDir', async () => {
  const root = await mkdtemp(join(tmpdir(), 'grover-apprun-resolver-'))
  const appDir = join(root, 'Grover.AppDir')
  ...
  const absoluteSymlink = join(binDir, 'grover')
  const relativeSymlink = join(relativeDir, 'grover')
...
test('plugin wrapper patches AppRun before delegating to the real output plugin', async () => {
  const root = await mkdtemp(join(tmpdir(), 'grover-appimage-plugin-'))
  const appDir = join(root, 'Grover.AppDir')
  const appRun = join(appDir, 'AppRun')
  ...
  const result = spawnSync(wrapper, [], {
    ...
    env: {
      ...process.env,
      APPDIR: appDir,
      GROVER_APPIMAGE_REAL_PLUGIN: realPlugin,
    },
  })
...
test('plugin wrapper keeps the delegated appimage plugin basename canonical', async () => {
  const root = await mkdtemp(join(tmpdir(), 'grover-appimage-tools-'))
  ...
  assert.equal(
    realPluginPath,
    join(root, 'grover-real-linuxdeploy-plugin-appimage', 'linuxdeploy-plugin-appimage.AppImage'),
  )
...
test('plugin wrapper bundles fcitx GTK3 input module before sealing AppImage', async () => {
  const root = await mkdtemp(join(tmpdir(), 'grover-appimage-fcitx-'))
  const appDir = join(root, 'Grover.AppDir')
  const wrapper = join(root, 'linuxdeploy-plugin-appimage.AppImage')
  const realPlugin = join(root, 'linuxdeploy-plugin-appimage.real.AppImage')
  const pluginMarker = join(root, 'plugin-ran')
  ...
  const result = spawnSync(wrapper, [], {
    ...
    env: {
      ...
      APPDIR: appDir,
      GROVER_APPIMAGE_REAL_PLUGIN: realPlugin,
      GROVER_FCITX_GTK3_IM_MODULE: hostModule,
      GROVER_FCITX_LIBRARY_DIR: hostLibraryDir,
    },
  })
