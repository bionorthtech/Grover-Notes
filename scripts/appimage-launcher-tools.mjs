...
export const REAL_APPIMAGE_PLUGIN_NAME =
  'grover-real-linuxdeploy-plugin-appimage/linuxdeploy-plugin-appimage.AppImage'
...
const WRAPPER_MARKER = 'Grover AppImage symlink launcher shim'
...
export function tauriToolsCacheDir(env = process.env) {
  if (env.GROVER_TAURI_TOOLS_DIR) {
    return resolve(env.GROVER_TAURI_TOOLS_DIR)
  }
...
# ${WRAPPER_MARKER}
PLUGIN_URL="\${GROVER_APPIMAGE_PLUGIN_URL:-${pluginUrl}}"
SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
REAL_PLUGIN="\${GROVER_APPIMAGE_REAL_PLUGIN:-"$SCRIPT_DIR/${REAL_APPIMAGE_PLUGIN_NAME}"}"
FCITX_GTK3_IM_MODULE="\${GROVER_FCITX_GTK3_IM_MODULE:-/usr/lib/x86_64-linux-gnu/gtk-3.0/3.0.0/immodules/im-fcitx5.so}"
FCITX_LIBRARY_DIR="\${GROVER_FCITX_LIBRARY_DIR:-/usr/lib/x86_64-linux-gnu}"
...
export async function validateAppImages(paths) {
  ...
  for (const appImage of paths.map((path) => resolve(path))) {
    const tempDir = await mkdtemp(join(tmpdir(), 'grover-appimage-'))
    try {
