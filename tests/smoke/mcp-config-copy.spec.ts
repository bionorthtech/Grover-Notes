...
test.describe('MCP config copy', () => {
  ...
  test('copies the active-vault MCP config from the Settings AI section', async ({ context, page }) => {
    ...
    const groverServer = parsedConfig.mcpServers.grover

    expect(groverServer.type).toBe('stdio')
    expect(groverServer.command).toBe('node')
    expect(groverServer.args[0]).toContain('mcp-server/index.js')
    expect(groverServer.env.VAULT_PATH).toBeUndefined()
    expect(groverServer.env.WS_UI_PORT).toBe('9711')
  })
})
