...
function mermaidBodyArtifacts(page: Page): Locator {
  return page.locator([
    'body > [data-grover-mermaid-render-host]',
    'body > [id^="grover-mermaid-"]',
    'body > [id^="dgrover-mermaid-"]',
    'body > [id^="igrover-mermaid-"]',
  ].join(', '))
}
...
async function setDocumentZoom(page: Page, percent: number): Promise<void> {
  await page.evaluate((zoomPercent) => {
    ...
    document.documentElement.style.setProperty('--grover-overlay-zoom-factor', String(factor))
    document.documentElement.style.setProperty('--grover-overlay-zoom-inverse', String(1 / factor))
    window.dispatchEvent(new Event('grover-zoom-change'))
  }, percent)
}
...
test('fullscreen Mermaid diagrams keep the active Grover surface in dark mode', async ({ page }) => {
  await openNote(page, 'Mermaid Reported')
  await expectRenderedDiagramCount(page, 1)
...
test('Mermaid diagram clicks stay stable while a vault reload settles', async ({ page }) => {
  ...
  await expectRenderedDiagramCount(page, 1)
  await expect(page.locator('#grover-fatal-render-error')).toHaveCount(0)
