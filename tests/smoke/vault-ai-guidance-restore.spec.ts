...
test('vault guidance restore command recovers missing managed guidance', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window, '__mockHandlers', {
      ...
      set(value) {
        ...
        ref.get_default_vault_path = () => '/Users/luca/Grover'
        ref.check_vault_exists = (args: { path: string }) => args.path === '/Users/luca/Grover'
  ...
  await expect(aiBadge).toHaveAttribute('title', /Grover guidance missing or broken/)

  await openCommandPalette(page)
  expect(await findCommand(page, 'Restore Grover AI Guidance')).toBe(true)
  await page.keyboard.press('Enter')

  await expect(page.getByText('Grover AI guidance restored')).toBeVisible()

  await aiBadge.click()
  await expect(page.getByTestId('status-ai-guidance-summary')).toHaveText('Grover guidance ready')
  await expect(page.getByTestId('status-ai-guidance-restore')).toHaveCount(0)
  await page.keyboard.press('Escape')

  await openCommandPalette(page)
  expect(await findCommand(page, 'Restore Grover AI Guidance')).toBe(false)
})
