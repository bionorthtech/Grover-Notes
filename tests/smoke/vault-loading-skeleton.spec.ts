...
async function installSlowVaultMock(page: Page): Promise<void> {
  await page.addInitScript((entries) => {
    ...
    localStorage.setItem('grover:ai-agents-onboarding-dismissed', '1')
    localStorage.setItem('grover:claude-code-onboarding-dismissed', '1')
...
test('slow vault open keeps the app shell usable while notes load @smoke', async ({ page }) => {
  ...
  await expect(page.locator('#grover-boot-diagnostics')).toHaveCount(0)
