...
`
  ...
  expect(rawAfterRichMode).not.toContain('@@GROVER_MATH')
  ...
  expect(reopenedRaw).not.toContain('@@GROVER_MATH')
})
...
test('imported financial Markdown does not leak math placeholder tokens', async ({ page }) => {
  ...
  await expect(page.locator('.bn-editor')).not.toContainText('GROVER_MATH_INLINE')
