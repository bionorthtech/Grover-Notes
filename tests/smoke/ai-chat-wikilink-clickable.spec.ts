...
test.describe('AI chat wikilink rendering', () => {
  test.beforeEach(async ({ page }) => {
    // Block vault API so mock entries are used (ensures "Build Grover App" exists)
    ...
    // Send a message to trigger mock response with [[Build Grover App]] and [[Matteo Cellini]]
  ...
  test('[[Note]] in AI response renders as clickable wikilink', async ({ page }) => {
    ...
    await expect(wikilink).toHaveText('Build Grover App')
    await expect(wikilink).toHaveAttribute('data-wikilink-target', 'Build Grover App')
