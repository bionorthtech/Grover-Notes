...
async function waitForDispatchBrowserMenuCommand(page: Page): Promise<void> {
  await page.waitForFunction(
    () => typeof window.__groverTest?.dispatchBrowserMenuCommand === 'function',
    undefined,
    { timeout: 5_000 },
  )
}
...
async function attemptTriggerMenuCommandInPage(commandId: string): Promise<boolean> {
  const triggerMenuCommand = window.__groverTest?.triggerMenuCommand
...
function dispatchMenuCommandFallbackInPage(commandId: string): void {
  const dispatchBrowserMenuCommand = window.__groverTest?.dispatchBrowserMenuCommand
  if (typeof dispatchBrowserMenuCommand !== 'function') {
    throw new Error('Grover test bridge is missing dispatchBrowserMenuCommand')
  }
...
export async function seedBlockNoteTable(
  ...
  await page.evaluate((widths) => {
    const bridge = window.__groverTest?.seedBlockNoteTable
    if (typeof bridge !== 'function') {
      throw new Error('Grover test bridge is missing seedBlockNoteTable')
    }
...
export async function seedAutoGitSavedChange(page: Page): Promise<void> {
  await page.evaluate(async () => {
    const bridge = window.__groverTest?.seedAutoGitSavedChange
    if (typeof bridge !== 'function') {
      throw new Error('Grover test bridge is missing seedAutoGitSavedChange')
    }
    await bridge()
  })
}
...
export async function openDeepLink(page: Page, url: string): Promise<void> {
  await page.evaluate((deepLinkUrl) => {
    const bridge = window.__groverTest?.openDeepLink
    if (typeof bridge !== 'function') {
      throw new Error('Grover test bridge is missing openDeepLink')
    }
    bridge(deepLinkUrl)
  }, url)
}
...
export async function dispatchShortcutEvent(
  ...
  await page.evaluate((eventInit) => {
    const bridge = window.__groverTest?.dispatchShortcutEvent
    if (typeof bridge !== 'function') {
      throw new Error('Grover test bridge is missing dispatchShortcutEvent')
    }
    bridge(eventInit)
  }, init)
}
...
export async function triggerShortcutCommand(
  ...
  await page.evaluate((payload) => {
    const bridge = window.__groverTest?.triggerShortcutCommand
    if (typeof bridge !== 'function') {
      throw new Error('Grover test bridge is missing triggerShortcutCommand')
    }
