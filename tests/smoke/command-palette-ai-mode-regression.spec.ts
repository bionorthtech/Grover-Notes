...
test.describe('Command palette AI mode regression', () => {
  ...
  test('keeps focus, supports inline chip edits, and survives selection deletion', async ({ page }) => {
    ...
    await expect(page.getByTestId('wikilink-menu')).toContainText('Build Grover App')

    await page.getByTestId('wikilink-menu').getByText('Build Grover App').click()
    await expect(aiInput.getByTestId('inline-wikilink-chip')).toContainText('Build Grover App')
    await expectCaretAfterChip(page)

    await page.keyboard.type(' essay')
    await expectNormalizedEditorText(aiInput, 'edit my Build Grover App essay')
