    ...
    Object.defineProperty(navigator, 'clipboard', {
      ...
      value: {
        writeText: async (text: string) => {
          ;(window as Window & { __groverCopiedText?: string }).__groverCopiedText = text
        },
      },
    })
  })
...
async function copiedText(page: Page): Promise<string> {
  return page.evaluate(() => (window as Window & { __groverCopiedText?: string }).__groverCopiedText ?? '')
}
...
test('command palette copies and opens a Grover item deep link', async ({ page }) => {
  ...
  expect(deepLink).toBe('grover://test-vault/project/alpha-project.md')
  ...
  await openDeepLink(page, 'grover://missing-vault/project/alpha-project.md')
