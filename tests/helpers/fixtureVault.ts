...
const CLAUDE_CODE_ONBOARDING_DISMISSED_KEY = 'grover:claude-code-onboarding-dismissed'
...
export function createFixtureVaultCopy(): string {
  const tempVaultDir = fs.mkdtempSync(path.join(os.tmpdir(), 'grover-test-vault-'))
      ...
      get_mcp_config_snippet: () => JSON.stringify({
        mcpServers: {
          grover: {
            type: 'stdio',
            command: 'node',
            args: ['/fixture/Grover/mcp-server/index.js'],
            env: {
              WS_UI_PORT: '9711',
            },
          },
