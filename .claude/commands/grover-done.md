...
Mark a Grover task as done: add completion comment, move to In Review, then self-dispatch the next task.
...
Run `/grover-next-task` to get the next task and start working on it immediately.

If there are no tasks, `/grover-next-task` will wait 10 minutes and retry automatically. Do NOT exit — stay alive and let it loop.
