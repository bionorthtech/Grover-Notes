...
test('clicking a note opens it in the editor with content', async ({ page }) => {
  // Click "Build Grover App" in the note list
  await page.locator('.note-list__item', { hasText: 'Build Grover App' }).click()
  await page.waitForTimeout(300)

  // Tab should appear and be active
  await expect(page.locator('.editor__tab--active')).toHaveText(/Build Grover App/)
...
test('editor shows markdown content with live preview', async ({ page }) => {
  ...
  await expect(page.locator('.cm-content')).toContainText('Build Grover App')
})
...
test('sidebar filter: People shows only Person entries', async ({ page }) => {
  ...
  await expect(page.locator('.note-list__title', { hasText: 'Build Grover App' })).not.toBeVisible()
})
...
test('sidebar filter: Events shows only Event entries', async ({ page }) => {
  ...
  await expect(page.locator('.note-list__count')).toHaveText('1')
  await expect(page.locator('.note-list__title', { hasText: 'Grover App Design Session' })).toBeVisible()
})
...
test('clicking a tab switches to it', async ({ page }) => {
  ...
  await page.locator('.editor__tab', { hasText: 'Build Grover App' }).click()
  await page.waitForTimeout(200)
...
test('inspector shows backlinks', async ({ page }) => {
  // Open a note that has backlinks (Build Grover App is referenced by Facebook Ads and Budget Allocation)
  await page.locator('.note-list__item', { hasText: 'Build Grover App' }).click()
  await page.waitForTimeout(300)
...
test('inspector shows git history', async ({ page }) => {
  ...
  await expect(page.locator('.inspector__commit-msg').first()).toContainText('26q1-grover-app')
})
