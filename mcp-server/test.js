...
before(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'grover-mcp-test-'))
...
describe('getNote', () => {
  ...
  it('should reject absolute paths outside the vault', async () => {
    await assertRejectsOutsideVault('grover-mcp-outside-', outsideNote => outsideNote)
  })

  it('should reject traversal paths outside the vault', async () => {
    await assertRejectsOutsideVault(
      'grover-mcp-traversal-',
...
describe('requireVaultPath', () => {
  ...
  it('rejects missing vault paths instead of falling back to ~/Grover', async () => {
    const configDir = await mkdtemp(path.join(os.tmpdir(), 'grover-mcp-empty-config-'))
    assert.throws(
  ...
  it('loads active mounted vault paths from Grover config when env is vault-neutral', async () => {
    const configDir = await mkdtemp(path.join(os.tmpdir(), 'grover-mcp-config-'))
    ...
    const configPath = path.join(configDir, 'com.grover.app', 'vaults.json')
...
async function connectMcpClient() {
  ...
  const client = new Client(
    { name: 'grover-mcp-test-client', version: '0.0.0' },
    { capabilities: {} },
  )
