#!/usr/bin/env node
/**
 * Grover MCP Server — lightweight vault tools for AI agents.
 *
 * These MCP tools provide Grover-specific capabilities alongside each
 ...
 *   - open_note: signal Grover UI to open a note as a tab
...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Get vault orientation for the active Grover vaults: entity types, AGENTS.md instructions, note count, folders, and recent notes.',
    annotations: LOCAL_READ_ONLY_TOOL_ANNOTATIONS,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'list_vaults',
    description: 'List the current active Grover vaults available to MCP tools, including whether each vault has AGENTS.md instructions.',
    annotations: LOCAL_READ_ONLY_TOOL_ANNOTATIONS,
    inputSchema: {
      type: 'object',
      properties: {},
  ...
  {
    name: 'open_note',
    description: 'Open a note in the Grover UI as a new tab. Use after creating or editing a note so the user can see it.',
    annotations: LOCAL_READ_ONLY_TOOL_ANNOTATIONS,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Visually highlight a UI element in Grover (editor, tab, properties panel, or note list). The highlight auto-clears after a short delay.',
    annotations: LOCAL_READ_ONLY_TOOL_ANNOTATIONS,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'refresh_vault',
    description: 'Trigger a vault rescan so new or modified files appear immediately in the Grover note list.',
    annotations: LOCAL_READ_ONLY_TOOL_ANNOTATIONS,
    inputSchema: {
      type: 'object',
      properties: {
...
function requestedVaultPath(args = {}) {
  ...
  if (!activeVaultPaths().includes(requested)) {
    throw new Error(`Vault is not active in Grover: ${requested}`)
  }
  return requested
}
...
function handleOpenNote(args) {
  ...
  return { content: [{ type: 'text', text: `Opening ${targetPath} in Grover` }] }
}
...
const server = new Server(
  { name: 'grover-mcp-server', version: '0.3.0' },
  { capabilities: { tools: {} } },
)
...
async function main() {
  ...
  console.error('Grover MCP server running (vaults resolved per call)')
}
