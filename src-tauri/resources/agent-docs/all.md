...
Grover asks whether you want to:
...
Grover stores app-level settings on the local machine. Your notes stay in the vault folder you choose.
...
Grover can show an optional AI agents prompt after a vault is open. It checks common local install locations for supported coding agents and gives you setup paths, but you can dismiss it and use Grover without AI.
...
The Getting Started vault is a small public sample vault hosted at [bionorthtech/grover-notes-getting-started](https://github.com/bionorthtech/grover-notes-getting-started).

It exists to show Grover's conventions without requiring you to restructure your own notes first.
...
When Grover clones the sample, it removes the remote from the local copy. This makes the sample vault disposable. You can edit it freely, commit locally, and delete it later.

To connect a vault to your own remote, use the bottom status bar remote chip or run `Add Remote` from the command palette.

Grover also repairs starter-vault guidance files when needed. `AGENTS.md` is the canonical guidance file, `CLAUDE.md` is kept as a compatibility shim, and `GEMINI.md` is only created when you explicitly restore Gemini guidance.
...
After you understand the sample, open your own vault. Grover does not require a special folder structure: a folder of Markdown files is enough to start. You can remove the sample from Grover's vault list later without deleting its files from disk.
...
Grover publishes desktop builds for macOS, Windows, and Linux. macOS is the primary day-to-day development target, with Windows and Linux builds supported through the release pipeline and fixed as platform issues are found.

## Download

Use the latest stable release unless you are intentionally testing pre-release builds:

- <a href="https://grover.md/download/" target="_self">Download the latest stable build</a>
- [Browse all GitHub releases](https://github.com/bionorthtech/grover-notes/releases)
- <a href="https://grover.md/releases/" target="_self">Read the release notes</a>
...
| Windows | Supported, early | NSIS installers are Authenticode-signed and updater bundles are Tauri-signed. Company-managed SmartScreen, Defender, or WDAC policies can still require IT approval of the Grover publisher before first install. |
...
Do not disable SmartScreen or Windows Security to install Grover. On a managed Windows device, validate that the downloaded installer has a valid Grover Authenticode signature, then install it through the normal Windows prompt. If company policy still blocks the first run because reputation is not yet established, ask IT to approve the Grover publisher or deploy the same signed installer through the approved software portal.
...
A Grover vault is a folder on disk. The folder can contain Markdown notes, attachments, type definitions, saved views, and Git metadata.

## Open An Existing Folder

Choose an existing folder if you already have Markdown notes. Grover scans `.md` files and uses frontmatter when it exists.
...
Choose a new empty folder if you want Grover conventions from the start. New notes and optional type definitions are created as Markdown files.
...
Grover works well with a plain folder of Markdown files. You can open, edit, organize, and search notes without making the vault a Git repository.

Git is recommended when you want local history, diff views, recovery, pull, push, and remote sync without a proprietary backend. If a vault is not already a repository, Grover can initialize one when you explicitly ask it to.
...
Grover has two AI paths: coding agents that can use tools to inspect and edit a vault, and direct model targets that answer in chat mode from note context.

## Coding Agents

The AI panel can stream supported local CLI agents through Grover's normalized event layer. Current targets include Claude Code, Codex, OpenCode, Pi, and Gemini CLI when they are installed on the machine.
...
Grover exposes an MCP server for external tools. The setup flow can write Grover's MCP entry into Claude Code, Gemini CLI, Cursor, and a generic MCP config path, and it can also copy the exact JSON snippet for manual setup.
...
Grover offers a rich editor for daily writing and a raw Markdown mode for exact file control. Both modes write back to the same Markdown file.
...
Grover starts with Markdown notes, but a vault can also contain images, PDFs, media files, whiteboards, and other local files.
...
Grover renders Mermaid diagrams in the editor while keeping the source in Markdown.
...
Grover can preview common image files, PDFs, and supported media files in the app. Files without an in-app preview can still be opened in the default system app.
...
If generated or local-only files are ignored by Git, Grover can hide them from notes, search, quick open, and folders. Use this when build artifacts or private local files should not behave like vault content.
...
Keep the Inbox small enough that it can be reviewed in one focused pass. Grover works best when capture is fast and organization is deliberate.
...
A note is a Markdown file with optional YAML frontmatter. Grover reads the first H1 as the primary title and keeps the file on disk as the durable representation.
...
Draft the public Grover docs and keep them close to code changes.
```

## Titles

The first H1 is the note title. Grover uses that title wherever the note is displayed: note lists, search results, wikilink suggestions, relationship pickers, tabs, and window titles.
...
Use `[[wikilinks]]` to connect notes from the body. Grover shows autocomplete suggestions while you type, and links can resolve by filename or title.
...
Properties are frontmatter fields that Grover can display, filter, and edit.

## Suggested Properties

Suggested properties are the fields Grover knows how to create quickly from the Properties panel. When a suggested property is missing, the panel shows a shortcut to add it with the right editor.
...
Date fields use Grover's picker, relationship fields can use wikilinks, and raw Markdown mode is available when you need direct control over YAML.
...
Grover supports default relationship fields out of the box: `belongs_to`, `has`, and `related_to`. It also detects custom relationship fields dynamically when they contain wikilinks.
...
Grover can show incoming links and inverse relationships, making it easier to navigate from a note to the rest of its context.
...
Grover does not infer type from folder location. Moving a file into another folder does not change its type.

## Prefer Types Over Folders

Types are the preferred way to group notes in Grover. Folders are supported for existing vaults and fallback organization, but Grover is built around types and relationships because they carry stronger meaning than file paths.

Use types for semantic groups such as Projects, People, Topics, Procedures, Events, and Essays. Use relationships to connect notes across those groups. This gives Grover better structure for navigation, filtering, properties, templates, and future automation than folder location alone.
...
Type documents can define empty properties and relationships. When you create a new note of that type, Grover shows placeholders for those fields so you can fill them in from the Properties panel.
...
A vault is the folder Grover reads and writes. The filesystem is the source of truth; the app state and cache are derived from files.
...
Grover should never become the only way to read your data.
...
If a folder is not a Git repository, Grover can initialize Git when you explicitly ask it to. It avoids initializing broad personal folders such as Desktop, Documents, or Downloads unless they are clearly dedicated vault folders.

## Multiple Vaults At The Same Time

Grover can load multiple registered vaults into one unified graph. Enable this from `Settings` -> `Vaults` -> `Use multiple vaults at the same time`.

After the option is enabled, open the bottom-left vault menu to include or exclude vaults from the graph. Included vaults appear together in note lists, search, quick open, backlinks, and wikilink navigation. Each note keeps a compact vault badge when Grover needs to disambiguate where it lives.
...
Grover can act as a lightweight Git client for a Git-enabled vault. You can manage commits and pushes yourself, or enable AutoGit to create conservative checkpoints after editing pauses or when the app is no longer active.
...
AutoGit is available in Settings for Git-enabled vaults. When enabled, Grover automatically commits and pushes saved local changes after an idle pause or after the app becomes inactive.
...
Grover does not store provider API keys in vault settings. Choose one of the supported key paths:
...
After adding a provider, use the test action in Settings. A successful test means Grover reached the endpoint and the model replied.
...
Make sure the remote repository exists and your system Git can authenticate to it. Grover uses system Git rather than storing provider-specific credentials.
...
Grover can pluralize type names in the sidebar. Turn this off in Settings if your type names should be shown exactly as written, or use `_sidebar_label` on a type document for an explicit label.
...
That action is what removes the note from Inbox. If auto-advance is enabled in Settings > Workflow, Grover opens the next Inbox item immediately after you mark the current note organized.
...
Grover gives you two ways to ask for AI help: open the AI panel for an ongoing conversation, or prompt directly from the editor with `Cmd+K` followed by a space.
...
If a coding agent is missing, install it and reopen Grover or switch to another target.
...
AI edits are file edits. Review them with Grover's diff and Git history before committing.
...
The command palette is the fastest way to move around Grover.
...
Media previews let you inspect vault files without leaving Grover.

## Open A File

Select an image, PDF, media file, or unsupported file from a folder or file list. Grover opens supported files in the app and offers an external-open action for files that should use the system default app.
...
When you paste or drop an image into a note, Grover copies it into the vault and references the copied file from Markdown.
...
Grover builds the outline from the current note's headings. The panel updates as the note changes and can jump to sections in the editor.
...
Prefer clear note titles and filenames. Grover's wikilink autocomplete helps you pick the right target while you type.
...
Those questions map naturally to Grover's type documents, relationship fields, Inbox, organized state, archive behavior, and custom views.
...
In Grover, these relationships can live in YAML frontmatter and point to other notes with wikilinks. That keeps the graph portable, searchable, and readable outside the app.
...
Grover supports that lifecycle directly: the Inbox holds captured notes, organizing a note marks it ready for normal views, and archiving hides old or obsolete notes from active surfaces while keeping them available.
...
Because Portent is file-friendly and portable, the same model can work across local Markdown vaults, note apps, docs tools, and agent-readable knowledge bases. Grover is the first intended implementation, but the spec is not tied to Grover internals.
...
Use it as-is, rename pieces to match your language, or treat it as a reference model for your own Grover setup.
...
Grover is free and open source, and any kind of help is useful. Pick the path that matches what you want to do.

## Sponsor Or Support

The best way to support Grover is to subscribe to [Refactoring](https://refactoring.fm/), Luca's newsletter and community about running good teams and shipping software with AI.

## Feature Requests

Use the [product board](https://grover.canny.io/) for feature ideas. Search first, upvote existing ideas, and create a new post when the request is genuinely new.

## Discussions

Use [GitHub Discussions](https://github.com/bionorthtech/grover-notes/discussions) for questions, conversations, show and tell, and broader community context.

## Contribute Code

Small, focused pull requests are welcome. Check the product board first so you build the right thing, then open a PR on [GitHub](https://github.com/bionorthtech/grover-notes/pulls). The [contributing guide](https://github.com/bionorthtech/grover-notes/blob/main/CONTRIBUTING.md) explains the local workflow.

## Report A Bug

Use [GitHub Issues](https://github.com/bionorthtech/grover-notes/issues) for bugs. Include what happened, what you expected, and clear reproduction steps. If you are reporting from inside Grover, use the Contribute panel to copy sanitized diagnostics and attach them to the issue.
...
Grover is not opinionated about folder structure. It finds notes recursively across the whole vault, stores new notes in the root by default, and uses types and relationships for real organization.
...
Grover works well with a flat vault. Folders are optional and can be useful for compatibility with other tools, but they are not required for people, projects, topics, or any other note category.

Type is not inferred from folder location. It comes from frontmatter, and relationships are expressed with wikilinks in fields. That is what Grover uses for the sidebar, Properties panel, search, custom views, and neighborhood navigation.
...
If the vault is a Git repository, `.git/` belongs to Git. Grover reads Git state but does not treat `.git/` as notes.
...
Grover uses conventions instead of a required schema.
...
You can add your own fields. If a field contains wikilinks, Grover can treat it as a relationship.
...
Grover publishes Stable and Alpha release metadata to GitHub Pages.
...
Grover is a desktop app built with Tauri. Releases currently target macOS, Windows, and Linux.
...
Grover can only launch local CLI agents that are installed and discoverable.

## Symptoms

- The AI panel says no supported agent is available.
- Claude Code or another agent works in one shell but not in Grover.
...
Desktop apps can inherit a different `PATH` from your interactive shell. Grover checks common install locations, but shell setup can still vary. Prefer installing CLI tools in standard locations or making them available from your login shell.
...
Grover uses system Git authentication. It does not manage provider passwords directly.

## Symptoms

- Push fails.
- Pull asks for credentials repeatedly.
- Remote fetch works in one terminal but not in Grover.
...
2. Confirm the base URL in Grover matches the server.
...
2. Open the conflict resolver if Grover presents it.
3. Review both sides.
...
Use this checklist when Grover cannot open or refresh a vault.

## Check The Folder

- Confirm the folder exists.
- Confirm the folder contains readable files.
- Confirm Grover has permission to access the folder.
