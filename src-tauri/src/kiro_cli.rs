...
fn write_mcp_json(config: KiroMcpConfig<'_>) -> Result<(), String> {
    ...
    servers["grover"] = serde_json::json!({
        "command": "node",
...
mod tests {
    ...
    fn write_mcp_json_creates_config() {
        ...
        assert_eq!(content["mcpServers"]["grover"]["command"], "node");
        assert_eq!(
            content["mcpServers"]["grover"]["args"][0],
            "/opt/mcp/index.js"
        );
        assert_eq!(
            content["mcpServers"]["grover"]["env"]["VAULT_PATH"],
            vault_path
        );
        assert_eq!(
    ...
    fn write_mcp_json_merges_preserving_existing_servers() {
        ...
        .unwrap();
        assert_eq!(content["mcpServers"]["grover"]["args"][0], "/new/index.js");
