...
fn runtime_resource_roots_for_env_and_exe(
    ...
    if let Some(appdir) = appdir {
        ...
        push_resource_root(&mut roots, appdir.join("usr/lib/grover"));
        push_resource_root(&mut roots, appdir.join("usr/lib/Grover"));
    }
    if let Some(local_app_data) = local_app_data {
        push_resource_root(&mut roots, local_app_data.join("Grover"));
        push_resource_root(&mut roots, local_app_data.join("grover"));
    }
...
mod tests {
    ...
    fn includes_windows_install_locations() {
        ...
        let install_dir = local_app_data.join("Grover");
        let roots =
        ...
        assert!(roots.contains(&local_app_data.join("grover")));
    ...
    fn includes_macos_app_bundle_resources_from_executable_path() {
        let executable = PathBuf::from("/Applications/Grover.app/Contents/MacOS/Grover");
