...
const VERSION_MARKER_FILE: &str = ".grover-version";
#[cfg(all(desktop, target_os = "linux"))]
...
fn stable_mcp_server_dir() -> Result<PathBuf, String> {
    dirs::data_dir()
        .map(|data_dir| data_dir.join("grover").join("mcp-server"))
...
mod tests {
    ...
    fn stable_mcp_server_dir_requires_marker_and_files() {
        ...
        let stable_dir = tmp.path().join("grover").join("mcp-server");
    ...
    fn replace_stable_server_dir_swaps_versioned_copy() {
        let tmp = tempfile::tempdir().unwrap();
        let source = create_server_dir(tmp.path());
        let target = tmp.path().join("grover").join("mcp-server");
