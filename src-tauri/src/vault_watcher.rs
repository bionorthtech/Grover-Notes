...
fn is_exact_temp_file_name(name: &str) -> bool {
    [".DS_Store", ".grover-rename-txn"].contains(&name)
}
...
mod tests {
    ...
    fn ignores_common_temporary_files() {
        ...
        assert!(!is_watchable_path(Path::new(".grover-rename-txn"), None));
    ...
    fn keeps_notes_assets_and_saved_views_watchable() {
        ...
        assert!(is_watchable_path(Path::new(".grover/views/work.yml"), None));
    }
