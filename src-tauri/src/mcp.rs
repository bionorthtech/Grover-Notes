...
fn verify_node_version(node: &Path) -> Result<(), String> {
    ...
    if major < 18 {
        return Err(format!(
            "Node.js 18+ is required for Grover MCP tools; found {}",
            raw_version.trim()
        ));
    }
...
fn verify_bun_version(bun: &Path) -> Result<(), String> {
    ...
    if major < 1 {
        return Err(format!(
            "Bun 1+ is required for Grover MCP tools; found {}",
            raw_version.trim()
        ));
    }
...
fn linux_package_mcp_server_dirs(root: &Path) -> Vec<PathBuf> {
    vec![
        root.join("Grover").join("mcp-server"),
        root.join("Grover").join("resources").join("mcp-server"),
        root.join("lib").join("Grover").join("mcp-server"),
        root.join("lib")
            .join("Grover")
            .join("resources")
            .join("mcp-server"),
...
/// Register Grover as an MCP server in external AI tool config files.
pub fn register_mcp(vault_path: &str) -> Result<String, String> {
    let _ = vault_path;
    let runtime = find_mcp_runtime().map_err(|e| {
        format!(
            "Node.js 18+ or Bun 1+ is required on PATH before Grover can register MCP tools: {e}"
        )
    })?;
    let server_dir = mcp_server_dir_for_registration()?;
    let index_js = server_dir.join("index.js").to_string_lossy().into_owned();
...
/// Insert or update the Grover entry in an MCP config file.
...
///
/// Returns `Installed` when the Grover entry exists for the active vault in
...
mod tests {
    ...
    fn assert_registered_grover_server(
    ...
    #[test]
    fn build_mcp_config_snippet_wraps_grover_server_entry() {
    ...
    fn mcp_server_dir_candidates_prefer_resource_root_before_linux_packages() {
        ...
        let linux_pos = candidates
            .iter()
            .position(|path| path == &PathBuf::from("/usr/local/Grover/mcp-server"))
            .unwrap();
    ...
    fn mcp_server_dir_candidates_include_linux_package_resource_roots() {
        let dev_path = Path::new("/repo/mcp-server");
        let resource_roots = vec![PathBuf::from("/opt/grover")];
        let candidates = mcp_server_dir_candidates(dev_path, &resource_roots);
        let expected = [
            PathBuf::from("/opt/grover/Grover/mcp-server"),
            PathBuf::from("/opt/grover/Grover/resources/mcp-server"),
            PathBuf::from("/opt/grover/lib/Grover/mcp-server"),
            PathBuf::from("/opt/grover/lib/Grover/resources/mcp-server"),
            PathBuf::from("/opt/grover/lib/grover/mcp-server"),
            PathBuf::from("/opt/grover/lib/grover/resources/mcp-server"),
            PathBuf::from("/usr/local/Grover/mcp-server"),
            PathBuf::from("/usr/local/Grover/resources/mcp-server"),
            PathBuf::from("/usr/local/lib/Grover/mcp-server"),
            PathBuf::from("/usr/local/lib/Grover/resources/mcp-server"),
            PathBuf::from("/usr/local/lib/grover/mcp-server"),
            PathBuf::from("/usr/local/lib/grover/resources/mcp-server"),
            PathBuf::from("/usr/lib/Grover/mcp-server"),
            PathBuf::from("/usr/lib/Grover/resources/mcp-server"),
            PathBuf::from("/usr/lib/grover/mcp-server"),
            PathBuf::from("/usr/lib/grover/resources/mcp-server"),
        ];
    ...
    fn mcp_server_dir_candidates_include_deb_capitalized_lib_root() {
        ...
        assert!(candidates.contains(&PathBuf::from("/usr/lib/Grover/mcp-server")));
    }

    #[test]
    fn mcp_server_dir_candidates_include_linux_appimage_resource_root() {
        let dev_path = Path::new("/repo/mcp-server");
        let resource_roots = vec![PathBuf::from("/tmp/.mount_grover/usr")];
        let candidates = mcp_server_dir_candidates(dev_path, &resource_roots);

        assert!(candidates.contains(&PathBuf::from(
            "/tmp/.mount_grover/usr/lib/grover/resources/mcp-server"
        )));
    }

    #[test]
    fn mcp_server_dir_candidates_include_runtime_dev_roots_when_build_path_is_stale() {
        let stale_dev_path = Path::new("/Users/runner/work/grover/grover/mcp-server");
        let current_dir = Path::new("/Users/luca/Workspace/grover");
        let candidates = mcp_server_dir_candidates_for(stale_dev_path, &[], Some(current_dir));

        assert!(candidates.contains(&PathBuf::from("/Users/luca/Workspace/grover/mcp-server")));
        assert!(candidates.contains(&PathBuf::from(
            "/Users/luca/Workspace/grover/src-tauri/resources/mcp-server"
        )));
    }
    ...
    fn mcp_server_dir_candidates_include_macos_bundle_resources() {
        ...
        assert!(candidates.contains(&PathBuf::from(
            "/Applications/Grover.app/Contents/Resources/mcp-server"
        )));
    }
    ...
    fn upsert_creates_new_config() {
        ...
        assert_registered_grover_server(
            &config,
            ExpectedMcpServer {
    ...
    fn remove_mcp_from_config_removes_primary_and_legacy_entries() {
        ...
        let config = serde_json::json!({
            "mcpServers": {
                "grover": { "command": "node", "args": ["/index.js"] },
                "grover": { "command": "node", "args": ["/legacy.js"] },
                "other-server": { "command": "other", "args": [] }
            }
        });
        std::fs::write(&config_path, serde_json::to_string(&config).unwrap()).unwrap();
