    ...
    let mut settings = serde_json::json!({
        "mcpServers": {
            "grover": {
                ...
                "description": "Grover active vault MCP server",
...
"#,
    ...
    #[test]
    fn safe_settings_include_grover_mcp_and_exclude_shell() {
        ...
        assert_eq!(json["mcpServers"]["grover"]["trust"], false);
    ...
    #[test]
    fn power_user_settings_trust_grover_and_allow_shell_discovery() {
        let settings = build_settings("/tmp/vault", &[], AiAgentPermissionMode::PowerUser).unwrap();
        let json: serde_json::Value = serde_json::from_str(&settings).unwrap();

        assert_eq!(json["mcpServers"]["grover"]["trust"], true);
