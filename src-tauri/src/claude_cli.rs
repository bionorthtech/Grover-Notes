...
"#,
    ...
    fn run_subprocess_closes_stdin_even_when_parent_stdin_pipe_is_open() {
        ...
        let mut child = Command::new(current_test_binary())
            ...
            .arg("--ignored")
            .arg("--nocapture")
            .env("GROVER_STDIN_PROBE_CHILD", "1")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
    ...
    fn stdin_probe_parent_child() {
        if std::env::var_os("GROVER_STDIN_PROBE_CHILD").is_none() {
            return;
        }
        ...
        std::env::set_var("GROVER_STDIN_PROBE_MOCK_CLAUDE_CHILD", "1");
        let mut events = vec![];
        let result = run_claude_subprocess(
            ClaudeSubprocessRequest {
                bin: &fake_bin,
        ...
        std::env::remove_var("GROVER_STDIN_PROBE_MOCK_CLAUDE_CHILD");
    ...
    fn stdin_probe_mock_claude_child() {
        if std::env::var_os("GROVER_STDIN_PROBE_MOCK_CLAUDE_CHILD").is_none() {
            return;
        }
