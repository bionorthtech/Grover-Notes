...
pub const GETTING_STARTED_REPO_URL: &str =
    "https://github.com/bionorthtech/grover-notes-getting-started.git";
...
/// Previous default AGENTS.md content seeded by Grover itself. Existing vaults
/// can still contain this exact text, so Grover treats it as managed content
/// that is safe to refresh automatically.
const STALE_AGENTS_MD: &str = r##"# AGENTS.md — Grover Vault

This is a [Grover](https://github.com/bionorthtech/grover-notes) vault - a folder of markdown files with YAML frontmatter forming a personal knowledge graph.

Keep edits compatible with Grover's current conventions. Prefer small, human-readable changes over heavy restructuring.
...
- Frontmatter properties that start with `_` are usually Grover-managed state. Leave them alone unless the user explicitly asks for them to change.
...
Grover still understands some legacy aliases such as `Is A`, but prefer `type:` for new or edited notes.
...
/// Older Grover-managed AGENTS.md content from before the `type:` migration.
/// Existing vaults can still contain this exact text, so Grover treats it as
/// managed content that is safe to refresh automatically.
const PRE_TYPE_AGENTS_MD: &str = r##"# AGENTS.md — Grover Vault
...
const OUTDATED_AGENTS_MARKERS: [&str; 3] = [
    "# AGENTS.md — Grover Vault",
    "Legacy `title:` frontmatter is still read as a fallback",
    "Grover still understands legacy aliases such as `Is A`.",
];
...
/// Describes Grover vault mechanics only; no user-specific structure.
...
This is a [Grover](https://github.com/bionorthtech/grover-notes) vault.

Keep this file focused on vault-specific conventions. For general Grover behavior, use the bundled Grover agent docs path provided by the app session context.

## Core conventions

- Notes are Markdown files.
- Use the first H1 as the note title. Grover uses this title in the note list, wikilinks, search, and other display surfaces.
...
- Grover reads notes recursively from all folders and stores new notes in the vault root by default.
...
Saved views live in `views/*.yml` and are written as YAML. Grover scans every `.yml` file in `views/`, and the filename is the stable view id, so use kebab-case filenames such as `active-projects.yml`.
...
- Relationship filters can use wikilinks in `value`, for example `"[[grover]]"`.
...
- Search the bundled Grover docs when the user asks how Grover works or when you need product behavior beyond these base conventions.
- Use Portent as the default best-practice model when the user asks how to improve, organize, or restructure the knowledge base. Combine Portent's types, relationships, and capture -> organize -> archive lifecycle with Grover's type documents, properties, Inbox, archive, and saved views.
...
pub(super) const LEGACY_AGENTS_MD: &str = r##"# AGENTS.md — Grover Vault
...
fn getting_started_repo_url() -> String {
    std::env::var("GROVER_GETTING_STARTED_REPO_URL")
        .or_else(|_| std::env::var("GROVER_GETTING_STARTED_REPO_URL"))
...
fn refresh_cloned_vault_config_files(vault_path: &Path) -> Result<(), String> {
    ...
    crate::git::git_commit(
        ...
        "Initialize Grover config files",
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn init_source_repo(path: &Path, agents_content: Option<&str>) {
        ...
        StdCommand::new("git")
            ...
            .unwrap();
        StdCommand::new("git")
            .args(["config", "user.email", "grover@app.local"])
            .current_dir(path)
            .output()
            .unwrap();
        StdCommand::new("git")
            .args(["config", "user.name", "Grover App"])
            .current_dir(path)
            .output()
            .unwrap();
        StdCommand::new("git")
    ...
    fn write_grover_config_files(path: &Path) {
    ...
    #[test]
    fn test_default_getting_started_repo_url_uses_grover_slug() {
        assert_eq!(
            GETTING_STARTED_REPO_URL,
            "https://github.com/bionorthtech/grover-notes-getting-started.git"
        );
    ...
    #[test]
    fn test_canonical_getting_started_path_rejects_plain_grover_folder() {
        let dir = tempfile::TempDir::new().unwrap();
        let default_path = dir.path().join("Getting Started");

        write_grover_config_files(&default_path);
    ...
    fn test_agents_refresh_detection_accepts_legacy_json_view_guidance() {
        let stale = r#"# AGENTS.md — Grover Vault
...
"#;
    ...
    #[test]
    fn test_agents_template_matches_current_grover_vault_conventions() {
        assert!(AGENTS_MD.starts_with("---\ntype: Note\n_organized: true\n---\n"));
        assert!(AGENTS_MD.contains("# AGENTS.md — Grover Vault"));
        ...
        assert!(AGENTS_MD.contains("Grover reads notes recursively from all folders"));
        assert!(AGENTS_MD.contains("Search the bundled Grover docs"));
