...
/// Return the cache directory. Override with `GROVER_CACHE_DIR` env var (for tests).
fn cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("GROVER_CACHE_DIR") {
...
mod tests {
    ...
    /// Serialize all cache tests that mutate the GROVER_CACHE_DIR env var.
    ...
    fn set_test_cache_dir(dir: &Path) {
        std::env::set_var("GROVER_CACHE_DIR", dir.to_string_lossy().as_ref());
    }
    ...
    fn test_to_relative_path_normalizes_aliases_and_separators() {
        assert_eq!(
            to_relative_path(
                "/tmp/grover-vault/projects\\active.md",
