...
fn test_scan_vault_folders_returns_tree() {
    ...
    std::fs::create_dir_all(dir.path().join("projects/grover")).unwrap();
    ...
    assert_eq!(projects.children[0].name, "grover");
    assert_eq!(projects.children[0].path, "projects/grover");
}
...
fn test_scan_vault_folders_excludes_hidden() {
    ...
    std::fs::create_dir_all(dir.path().join(".grover")).unwrap();
