...
fn test_parse_full_frontmatter_identity() {
    let dir = TempDir::new().unwrap();
    let entry = parse_test_entry(&dir, "grover.md", FULL_FM_CONTENT);
    assert_eq!(entry.title, "Grover Project");
    assert_eq!(entry.is_a, Some("Project".to_string()));
    assert_eq!(entry.filename, "grover.md");
}
...
fn test_parse_full_frontmatter_lists() {
    let dir = TempDir::new().unwrap();
    let entry = parse_test_entry(&dir, "grover.md", FULL_FM_CONTENT);
    assert_eq!(entry.aliases, vec!["Grover", "Castle in the Sky"]);
