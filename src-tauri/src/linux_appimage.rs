...
const GROVER_FCITX_IMMODULES_CACHE_FILE: &str = "grover-appimage-fcitx5-immodules.cache";
const COLRV1_EMOJI_FONT_FILE: &str = "Noto-COLRv1.ttf";
#[cfg(all(desktop, target_os = "linux"))]
const GROVER_COLRV1_FONTCONFIG_FILE: &str = "grover-appimage-no-colrv1-emoji.conf";
...
fn fcitx_immodules_cache_contents(module_path: &std::path::Path) -> String {
    format!(
        r#"# Grover AppImage GTK input method modules
"{}"
    ...
    E: FnMut(&str) -> bool,
{
    ...
    if has_non_empty_env(&mut get_var, "LD_PRELOAD")
        || get_var("GROVER_APPIMAGE_WAYLAND_PRELOAD_ATTEMPTED").is_some_and(|value| value == "1")
    {
        return None;
    }
...
fn apply_fcitx_gtk_im_module_file() {
    let Some(cache_path) = fcitx_immodules_cache_file_path() else {
        eprintln!("Grover AppImage fcitx GTK module skipped: failed to resolve cache directory");
        return;
    };
    ...
    if let Err(error) = std::fs::create_dir_all(parent) {
        eprintln!("Grover AppImage fcitx GTK module skipped: failed to prepare cache ({error})");
        return;
    }

    if let Err(error) = std::fs::write(&env_override.cache_path, env_override.cache_contents) {
        eprintln!("Grover AppImage fcitx GTK module skipped: failed to write cache ({error})");
        return;
    }
...
fn apply_colrv1_emoji_font_guard() {
    ...
    let Some(config_path) = colrv1_fontconfig_file_path() else {
        eprintln!("Grover AppImage COLRv1 font guard skipped: failed to resolve cache directory");
        return;
    };
    ...
    if let Err(error) = std::fs::create_dir_all(parent) {
        eprintln!("Grover AppImage COLRv1 font guard skipped: failed to prepare cache ({error})");
        return;
    }

    if let Err(error) = std::fs::write(&config_path, colrv1_emoji_fontconfig_contents(&font_path)) {
        eprintln!("Grover AppImage COLRv1 font guard skipped: failed to write config ({error})");
        return;
    }
...
fn colrv1_fontconfig_file_path() -> Option<std::path::PathBuf> {
    ...
    Some(cache_dir.join("grover").join(GROVER_COLRV1_FONTCONFIG_FILE))
}
...
fn fcitx_immodules_cache_file_path() -> Option<std::path::PathBuf> {
    ...
    Some(
        cache_dir
            .join("grover")
            .join(GROVER_FCITX_IMMODULES_CACHE_FILE),
    )
}
...
fn apply_wayland_client_preload() {
    ...
    let exe = match launched_appimage_path() {
        Ok(exe) => exe,
        Err(e) => {
            eprintln!("Grover AppImage Wayland preload skipped: {e}");
            return;
        }
    };
    ...
    let error = std::process::Command::new(exe)
        ...
        .env("GROVER_APPIMAGE_WAYLAND_PRELOAD_ATTEMPTED", "1")
        .exec();
    eprintln!("Grover AppImage Wayland preload skipped: failed to re-exec ({error})");
}
...
mod tests {
    ...
    fn appimage_wayland_fcitx_env(key: &str, gtk_im_module: Option<&str>) -> Option<String> {
        match key {
            "APPIMAGE" => Some("/tmp/Grover.AppImage".to_string()),
    ...
    fn startup_env_overrides_preserve_explicit_user_setting_per_variable() {
        let overrides = startup_env_overrides_with(|key| match key {
            "APPDIR" => Some("/tmp/.mount_Grover".to_string()),
    ...
    fn fcitx_module_file_override_points_gtk_to_bundled_appimage_module() {
        let appdir = std::path::Path::new("/tmp/.mount_Grover");
        let module_path = appdir.join(FCITX_GTK3_IM_MODULE_RELATIVE_PATH);
        let cache_path = std::path::PathBuf::from("/tmp/grover/immodules.cache");
    ...
    fn fcitx_module_file_override_preserves_explicit_module_cache() {
        let env_override = fcitx_gtk_im_module_file_override_with(
            ...
            |_| true,
            std::path::PathBuf::from("/tmp/grover/immodules.cache"),
        );
