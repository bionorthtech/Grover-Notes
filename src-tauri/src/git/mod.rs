...
const DEFAULT_GITIGNORE: &str = "# Grover app files (machine-specific, never commit)\n\
.grover/settings.json\n\
\n\
# macOS\n\
.DS_Store\n\
.AppleDouble\n\
...
pub(crate) fn ensure_author_config(dir: &Path) -> Result<(), String> {
    for (key, fallback) in [("user.name", "Grover"), ("user.email", "vault@grover.md")] {
...
mod tests {
    ...
    fn test_ensure_gitignore_creates_file() {
        ...
        assert!(content.contains(".grover/settings.json"));
    }
    ...
    fn test_init_repo_creates_initial_commit_when_signing_is_misconfigured() {
        ...
        git_command()
            ...
            .unwrap();
        git_command()
            .args(["config", "gpg.program", "/missing/grover-test-gpg"])
            .current_dir(&vault)
            .output()
            .unwrap();
    ...
    fn test_init_repo_creates_gitignore() {
        ...
        assert!(
            content.contains(".grover/settings.json"),
            ".gitignore should exclude settings.json"
        );
        // Cache is now stored outside the vault — no need for .gitignore entry
        assert!(
            !content.contains(".grover-cache.json"),
            ".gitignore should NOT contain .grover-cache.json (cache is external)"
        );
    }
