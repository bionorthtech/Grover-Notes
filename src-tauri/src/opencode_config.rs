...
"#,
    ...
    #[test]
    fn config_includes_permissions_and_grover_mcp_server() {
        if let Ok(config) = build_config(
            ...
            assert_eq!(
                (
                    ...
                    json["mcp"]["grover"]["type"].as_str(),
                    json["mcp"]["grover"]["command"][0].as_str(),
                    json["mcp"]["grover"]["environment"]["VAULT_PATH"].as_str(),
                    json["mcp"]["grover"]["command"][1]
                        .as_str()
            ...
            assert_eq!(
                json["mcp"]["grover"]["environment"]["VAULT_PATHS"],
                r#"["/tmp/vault"]"#
            );
            assert!(json["mcp"]["grover"]["command"][1]
                .as_str()
                .unwrap()
                .ends_with("index.js"));
        }
