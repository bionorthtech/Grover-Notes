...
fn validate_git_init_target(vault_path: &str) -> Result<(), String> {
    ...
    if is_broad_personal_folder(path) && !has_grover_vault_marker(path) {
        return Err(format!(
...
#[cfg(desktop)]
fn has_grover_vault_marker(path: &std::path::Path) -> bool {
...
mod tests {
    ...
    fn init_git_repo_allows_named_vault_subfolder_under_documents() {
        let dir = TempDir::new().unwrap();
        let vault = dir.path().join("Documents").join("Grover");
