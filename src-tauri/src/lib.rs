    ...
    F: FnOnce(PathBuf) + Send + 'static,
{
    ...
    spawn_background_task("grover-startup-tasks", move || task(vault_path));
    true
}

#[cfg(desktop)]
fn spawn_startup_tasks() {
    let Some(vault_path) = dirs::home_dir().map(|h| h.join("Grover")) else {
        return;
    };
...
fn spawn_initial_ws_bridge_sync(app: &tauri::App) {
    ...
    spawn_background_task("grover-ws-bridge-startup", move || {
