...
fn build_mcp_config_from_base(
    ...
    let mcp_server = grover_mcp_server_config(vault_path, vault_paths)?;
    ...
    servers.insert("grover".into(), mcp_server);

    serde_json::to_string(&config)
        .map_err(|error| format!("Failed to serialize Pi MCP config: {error}"))
}

fn grover_mcp_server_config(vault_path: &str, vault_paths: &[String]) -> Result<Value, String> {
...
mod tests {
    ...
    fn assert_seeded_pi_mcp_config(mcp: serde_json::Value) {
        ...
        assert_eq!(
            mcp["mcpServers"]["grover"]["env"]["VAULT_PATH"],
            "/tmp/vault"
        );
    }

    #[test]
    fn mcp_config_includes_grover_server_for_active_vault() {
        if let Ok(config) = build_mcp_config(
            ...
            assert_grover_mcp_env(&json);
            assert_grover_mcp_args(&json);
        }
    }

    fn assert_base_mcp_config(json: &serde_json::Value) {
        assert_eq!(json["settings"]["toolPrefix"], "none");
        assert_eq!(json["mcpServers"]["grover"]["command"], "node");
        assert_eq!(json["mcpServers"]["grover"]["lifecycle"], "lazy");
        assert_eq!(json["mcpServers"]["grover"]["directTools"], true);
    }

    fn assert_grover_mcp_env(json: &serde_json::Value) {
        assert_eq!(
            json["mcpServers"]["grover"]["env"]["VAULT_PATH"],
            "/tmp/vault"
        );
        assert_eq!(
            json["mcpServers"]["grover"]["env"]["VAULT_PATHS"],
            r#"["/tmp/vault"]"#
        );
        assert_eq!(json["mcpServers"]["grover"]["env"]["WS_UI_PORT"], "9711");
    }

    fn assert_grover_mcp_args(json: &serde_json::Value) {
        assert!(json["mcpServers"]["grover"]["args"][0]
            .as_str()
            .unwrap()
            .ends_with("index.js"));
    }
