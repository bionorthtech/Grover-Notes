    ...
    F: FnMut(AiAgentStreamEvent),
{
    let last_message_dir = tempfile::Builder::new()
        .prefix("grover-codex-last-message-")
        .tempdir()
...
fn build_codex_args(
    ...
    let mut args = vec![
        ...
        "-c".into(),
        codex_config_string("mcp_servers.grover.command", &node_path.to_string_lossy()),
        "-c".into(),
        codex_config_string_list("mcp_servers.grover.args", &[mcp_server_path.as_str()]),
        "-c".into(),
...
fn codex_mcp_env_config(request: &AgentStreamRequest) -> String {
    ...
    format!(
        r#"mcp_servers.grover.env={{VAULT_PATH="{}",VAULT_PATHS="{}",WS_UI_PORT="9711"}}"#,
...
mod tests {
    ...
    fn build_codex_args_can_request_last_message_output_file() {
        if let Ok(args) = build_codex_args(
            ...
            Some(Path::new("/tmp/grover-codex-last-message.txt")),
        ) {
            assert!(args.windows(2).any(|window| window
                == [
                    "--output-last-message",
                    "/tmp/grover-codex-last-message.txt",
                ]));
        }
    }
    ...
    fn build_codex_args_uses_resolved_mcp_node_and_ui_bridge_env() {
        ...
        let command_override = args
            .iter()
            .find(|arg| arg.starts_with("mcp_servers.grover.command="))
            .expect("Codex should receive a transient Grover MCP command");

        assert!(
            !command_override.ends_with(r#""node""#),
            "Codex MCP command should use Grover's resolved Node path, got {command_override}"
        );
        assert!(
...
"#,
        ...
        let command = build_codex_command(
            ...
            vec![
                "exec".to_string(),
                "-c".to_string(),
                r#"mcp_servers.grover.command="C:\\Program Files\\node.exe""#.to_string(),
            ],
            "Summarize".into(),
            "/tmp/vault",
        )
...
"#,
    ...
    fn run_codex_agent_stream_closes_stdin_even_when_parent_stdin_pipe_is_open() {
        ...
        let mut child = std::process::Command::new(current_test_binary())
            ...
            .arg("--ignored")
            .arg("--nocapture")
            .env("GROVER_CODEX_STDIN_PROBE_PARENT_CHILD", "1")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
    ...
    fn codex_stdin_probe_parent_child() {
        if std::env::var_os("GROVER_CODEX_STDIN_PROBE_PARENT_CHILD").is_none() {
            return;
        }
