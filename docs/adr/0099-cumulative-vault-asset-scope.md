...
**Grover accumulates Tauri asset protocol access for vault roots loaded during the current app session and never forbids a previously loaded vault root at runtime.**
...
- The runtime asset scope remains narrower than global filesystem access because only vault roots that Grover has loaded are added.
