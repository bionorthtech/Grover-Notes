...
- Filter YAML files that use `regex: true` require Grover ≥ this version to evaluate correctly; older versions silently ignore the flag (falling back to `regex: false` default via `#[serde(default)]`).
