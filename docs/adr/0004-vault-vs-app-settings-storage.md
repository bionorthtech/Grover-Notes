...
As features were added, there was recurring ambiguity about where to persist configuration and state: in the vault (as frontmatter in `.md` files) or in app settings (`~/.config/com.grover.app/settings.json` / localStorage). Without a clear rule, some decisions were inconsistent.
