...
- **React-rendered Linux chrome with shared command IDs** (chosen): keeps Linux visually aligned with Grover's existing shell and preserves one command-routing model across keyboard shortcuts, menu clicks, and QA helpers. Cons: Grover now owns Linux window chrome behavior directly.
...
- Linux main windows and detached note windows now present one consistent titlebar surface controlled by Grover.
...
- Grover now owns Linux resize handles, maximize/minimize/close behavior, and titlebar drag-region behavior in the renderer, so regressions in those surfaces require direct tests.
