...
- **Separate `.grover/favorites.json` file**: Central list of favorite paths. Simpler reorder (one file write). Cons: not portable if `.grover/` is gitignored; path references break on rename.
