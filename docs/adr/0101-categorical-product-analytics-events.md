...
Grover needs richer product telemetry for feature behavior while preserving the privacy bar expected from a local-first notes app.
...
- Re-evaluate if Grover ever needs a broader telemetry taxonomy or stronger compile-time guarantees around event schemas.
