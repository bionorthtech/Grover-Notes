...
Grover already keeps heavy filesystem and subprocess work off the Tauri window-creation path, but that alone does not protect against a different startup failure mode: the desktop WebView can render the static HTML shell while the React app never becomes interactive.
...
Grover needs a startup contract that distinguishes “HTML painted” from “frontend actually became interactive”, and a bounded recovery path when that contract is not satisfied.

## Decision

**Grover uses a Tauri-only frontend-readiness watchdog that reloads the WebView at most once if React never reports startup readiness.**
...
- `sessionStorage` tracks whether the startup reload was already attempted so Grover does not loop forever
...
- Grover now distinguishes successful frontend startup from merely rendering the HTML shell.
...
- If a startup failure persists after one retry, Grover still surfaces the broken state instead of hiding a deeper bug behind repeated reloads.
