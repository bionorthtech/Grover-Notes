...
ADR-0018 established CodeScene code-health gates so Grover could block regressions before code reached `main`. Since then, the codebase has improved materially and the tracked baseline in `.codescene-thresholds` has been ratcheted above the original 9.50 / 9.31 minimums.
...
- ADRs must be superseded again if Grover makes another meaningful policy jump in CodeScene thresholds.
