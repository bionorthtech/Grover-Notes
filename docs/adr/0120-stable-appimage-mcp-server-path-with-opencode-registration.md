...
On Linux AppImage startup, Grover extracts the bundled `mcp-server/` directory to `~/.local/share/grover/mcp-server/`. The extracted directory is version-gated by a `.grover-version` marker. Extraction runs on first launch or after an app version change, uses a staging directory plus rename, and uses a process lock so concurrent app launches do not write the stable directory at the same time.
...
OpenCode is added to durable MCP registration and removal. Grover writes an OpenCode-specific entry under the top-level `mcp` key using:
...
OpenCode registration remains vault-neutral. It does not write `VAULT_PATH`; the Node MCP server resolves active mounted workspaces from Grover state per ADR-0119.
