...
- Automated QA can trigger menu-owned commands deterministically through the shared `window.__groverTest.triggerMenuCommand()` bridge in browser runs and through the native `trigger_menu_command` Tauri command in desktop runs.
