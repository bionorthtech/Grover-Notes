...
**Custom views are stored as `.yml` files in `.grover/views/` within the vault root.** Each file defines a named view with filter conditions, optional icon/color, and sort preferences.
...
- `.grover/views/` directory auto-created on first view save. Already excluded from vault scanning via `HIDDEN_DIRS`.
