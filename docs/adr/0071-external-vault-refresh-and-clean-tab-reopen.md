...
Grover now routes post-pull refreshes and AI-agent file modifications through the same `refreshPulledVaultState()` helper.
...
- Grover now treats "refresh after external mutation" as a first-class synchronization concern rather than a per-feature fix.
