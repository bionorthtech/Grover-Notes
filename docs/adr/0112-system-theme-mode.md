...
ADR-0081 introduced Grover's internal app-owned light and dark theme runtime and deliberately deferred system-follow mode. That kept the first dark-mode release small, but users now need Grover to match the operating system appearance automatically, including scheduled macOS light/dark changes.
...
**Grover now treats `system` as a persisted theme preference that resolves to the current OS light/dark appearance at runtime.**
