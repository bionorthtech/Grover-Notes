...
- Gemini Safe excludes `run_shell_command`; Gemini Power User intentionally uses `yolo` with trusted transient Grover MCP settings.
...
- If Grover adds a stronger warning or dangerous mode later, it needs a separate ADR and UI language.
