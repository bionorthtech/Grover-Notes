...
ADR-0086 introduced the `FilePreview` path for image binaries while keeping binary files as ordinary `VaultEntry` records. The same file-first model should now cover PDFs, because asset-heavy vaults often mix screenshots, diagrams, and document exports that users need to inspect without leaving Grover.

## Decision

**Grover previews supported image and PDF files in the editor pane while keeping them as ordinary binary vault files.**
...
- Broken PDFs may rely on the webview's own renderer failure state, but the surrounding Grover preview chrome still provides reveal, copy path, and default-app actions.
