...
Users with folder-based workflows (PARA, Zettelkasten with folders, project directories) could not see or filter notes by directory. The vault scanner silently ignored all subdirectory `.md` files, making Grover unsuitable for vaults with any folder structure.
...
Hidden directories (names starting with `.`, plus `.git` and `.grover`) are excluded from both scanning and the folder tree.
...
* All `.md` files in the vault are now indexed regardless of depth — vaults with many non-note `.md` files (e.g. node_modules) will see spurious entries. Mitigation: hidden directories are already excluded; users can add a `.groverignore` in the future if needed.
