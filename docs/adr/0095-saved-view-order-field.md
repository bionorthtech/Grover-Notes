...
Saved Views already persist as user-editable YAML files in the vault and sync through Git. Filename ordering was stable, but it forced users to rename files just to change sidebar order and gave Grover no durable way to support drag reordering, move actions, or keyboard-first ordering controls.
...
**Each Saved View may store an optional top-level `order` number in its YAML definition, and Grover sorts views by that value before falling back to filename.**
...
- Reordering can touch multiple view files in one action because Grover normalizes the sequence.
