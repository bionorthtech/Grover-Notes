...
Future additions to `AppPreferencesProvider` should stay small, renderer-local, and read-focused. If Grover starts moving writable domain state, async workflows, or large derived objects into context, that needs a new ADR rather than quietly expanding this pattern.
