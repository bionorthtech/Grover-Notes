...
- Cache file stored outside the vault at `~/.grover/cache/<vault-hash>.json` — never pollutes the user's git repo.
...
- Legacy `.grover-cache.json` files inside the vault are auto-migrated and deleted on first run.
