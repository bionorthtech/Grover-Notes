...
The AI panel used to behave like another right-side editor panel. That kept the agent UI inside the main Grover window even when the user undocked it, so the "floating" surface could not be moved to another macOS space or placed beside Grover as a real window.
