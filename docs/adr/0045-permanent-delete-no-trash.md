...
The confirmation modal is the sole safety gate. No soft-delete, no Trash view, no auto-purge scheduler, no `.grover/purge.log`.
...
- **Option C — `.grover/deleted/` archive folder**: custom recovery mechanism inside vault. Downside: clutters vault, users wouldn't know to look there, still requires manual cleanup.
