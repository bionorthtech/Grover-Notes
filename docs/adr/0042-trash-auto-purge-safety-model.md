...
**Auto-purge trashed notes older than 30 days on app launch and window focus (max once per hour), using OS trash (`trash::delete`) for soft-deletion, with mandatory 5-point safety validation per file and an audit log at `.grover/purge.log`.**
...
Every purge run appends to `.grover/purge.log` with timestamp, files checked count, files purged count, and each purged file path. Users can inspect this file to audit what was deleted and when.
...
- **Option C — Move to `.grover/purged/` archive folder**: custom recovery mechanism, but clutters vault directory and users wouldn't know to look there.
