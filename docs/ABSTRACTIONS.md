...
External content is archived as ordinary typed notes, never a proprietary object. `src/lib/ingest/` defines a single `SourceNote` model (`title`, `source`, `url`, `author`, `body`, `assets[]`) and pure transforms that turn each input into one: a Reddit thread `.json`, a Discourse topic, or a raw HTML web clip. `detectAndTransform` sniffs the format; `buildSourceNoteMarkdown` serializes the note with provenance frontmatter (`type`, `source`, `url`, `author`, `captured_at`) so it flows into the typed graph, query engine, vault health, and AI like any other note. Imported notes land in `Sources/` with a `<source>-<slug>` filename.
